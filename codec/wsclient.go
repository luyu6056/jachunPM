...
func DailWebSocket(Url string) (c net.Conn, err error) {
	...
	for {
		...
		_, res, err := http.Request.Parsereq(client.readbuf.Bytes())
		if err != nil {
			return nil, err
		}
		if res != nil {
	...
	if ComputeAcceptKey(challengeKey) != http.Header("Sec-WebSocket-Accept") {
		return nil, errors.New("websocket握手失败")
	}
	client.readbuf.Reset()
	client.wsconn = &WSconn{Http: http, Write: client.c.AsyncWrite, ReadFinal: true, readbuf: &libraries.MsgBuffer{}}
	go func() {
		for {
