...
function loadBranches(product)
{
    ...
    {
        var length = $('#productsBox .input-group').length;
        $('#productsBox .row').append('<div class="col-sm-4">' + $('#productsBox .col-sm-4:last').html() + '</div>');
        if($('#productsBox .input-group:last select').length >= 2) $('#productsBox .input-group:last select:last').remove();
    ...
    if($inputgroup.find('select').length >= 2) $inputgroup.removeClass('has-branch').find('select:last').remove();
    if($inputgroup.find('.chosen-container').length >= 2) $inputgroup.find('.chosen-container:last').remove();
...
{
    if($('#plansBox').length == 0) return false;
...
$(function()
{
    $(document).on('click', '.task-toggle', function(e)
    {
        ...
        var child=$toggle.closest('[data-ride="table"]').find('tr.parent-' + id);
        
        child.each(function(index, el) {
            var childId=$(this).data('id');
            var $childToggle=$(this).find('.task-toggle');
            var childIsCollapsed=$childToggle.toggleClass('collapsed').hasClass('collapsed');
            if(childIsCollapsed==isCollapsed) $childToggle.closest('[data-ride="table"]').find('tr.parent-' + childId).toggle(!isCollapsed);
        });
