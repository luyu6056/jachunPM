...
func get_block_getblockdata(data *TemplateData) (err error) {
	...
	case "printStatisticBlock":
		...
		default:
			libraries.DebugLog("printStatisticBlock未处理" + block.Module)
			data.ws.WriteString("printStatisticBlock未处理" + block.Module)
		}
	case "printListBlock":
		switch block.Module {
		case "project":
		...
		default:
			libraries.DebugLog("printListBlock未处理" + block.Module)
			data.ws.WriteString("printListBlock未处理" + block.Module)
		}
	case "printOverviewBlock":
		...
		default:
			libraries.DebugLog("printOverviewBlock未处理" + block.Module)
			data.ws.WriteString("printOverviewBlock未处理" + block.Module)
		}
	case "printTaskBlock":
...
func printProjectStatisticBlock(data *TemplateData) (err error) {
	...
	for key, project := range projectMap {
		...
		project["progress"] = 0
		if project["totalConsumed"].(float64) > 0 || project["totalConsumed"].(float64) > 0 {
			project["progress"] = libraries.Round(project["totalConsumed"].(float64)/(project["totalConsumed"].(float64)+project["totalConsumed"].(float64)), 2) * 100
		}
