...
function(a) {
	a.extend({
		tablesorter: new function() {
			function S() {
				var d = arguments[0],
					f = 1 < arguments.length ? Array.prototype.slice.call(arguments) : d;
				if ("undefined" !== typeof console && "undefined" !== typeof console.log) {
					console[/error/i.test(d) ? "error" : /warn/i.test(d) ? "warn" : "log"](f)
				} else {
					alert(f)
				}
			}
			function s(d, f) {
				S(d + " (" + ((new Date).getTime() - f.getTime()) + "ms)")
			}
			function P(d) {
				for (var f in d) {
					return !1
			...
			function Q(d, f, z) {
				...
				var p, x = d.config,
					v = x.textExtraction || "",
					n = "",
					n = "basic" === v ? a(f).attr(x.textAttribute) || f.textContent || f.innerText || a(f).text() || "" : "function" === typeof v ? v(f, d, z) : "function" === typeof(p = R.getColumnData(d, v, z)) ? p(f, d, z) : f.textContent || f.innerText || a(f).text() || "";
				return a.trim(n)
			}
			function w(T) {
				var U = T.config,
					O = U.$tbodies = U.$table.children("tbody:not(." + U.cssInfoBlock + ")"),
					L, N, D, K, J, I, M, v, H, E = 0,
					C = "",
					B = O.length;
				if (0 === B) {
					return U.debug ? S("Warning: *Empty table!* Not building a parser cache") : ""
				}
				U.debug && (H = new Date, S("Detecting parsers for each column"));
				for (N = []; E < B;) {
					L = O[E].rows;
					if (L[E]) {
						for (D = U.columns, K = 0; K < D; K++) {
							J = U.$headers.filter('[data-column="' + K + '"]:last');
							I = R.getColumnData(T, U.headers, K);
							v = R.getParserById(R.getData(J, I, "sorter"));
							M = "false" === R.getData(J, I, "parser");
							U.empties[K] = R.getData(J, I, "empty") || U.emptyTo || (U.emptyToBottom ? "bottom" : "top");
							U.strings[K] = R.getData(J, I, "string") || U.stringTo || "max";
							M && (v = R.getParserById("no-parser"));
							if (!v) {
								U: {
									J = T;
									I = L;
									M = -1;
									v = K;
									for (var G = void 0, f = R.parsers.length, d = !1, n = "", G = !0;
									"" === n && G;) {
										M++, I[M] ? (d = I[M].cells[v], n = Q(J, d, v), J.config.debug && S("Checking if value was empty on row " + M + ", column: " + v + ': "' + n + '"')) : G = !1
									}
									for (; 0 <= --f;) {
										if ((G = R.parsers[f]) && "text" !== G.id && G.is && G.is(n, J, d)) {
											v = G;
											break U
										}
									}
									v = R.getParserById("text")
								}
							}
							U.debug && (C += "column:" + K + "; parser:" + v.id + "; string:" + U.strings[K] + "; empty: " + U.empties[K] + "\n");
							N[K] = v
						}
					}
					E += N.length ? B : 1
				}
				U.debug && (S(C ? C : "No parsers detected"), s("Completed detecting parsers", H));
				U.parsers = N
			}
			function o(G) {
				var H, D, B, C, d, z, x, v, E, I, n, f = G.config,
					K = f.$table.children("tbody"),
					J = f.parsers;
				f.cache = {};
				f.totalRows = 0;
				if (!J) {
					return f.debug ? S("Warning: *Empty table!* Not building a cache") : ""
				}
				f.debug && (v = new Date);
				f.showProcessing && R.isProcessing(G, !0);
				for (d = 0; d < K.length; d++) {
					if (n = [], H = f.cache[d] = {
						normalized: []
					}, !K.eq(d).hasClass(f.cssInfoBlock)) {
						E = K[d] && K[d].rows.length || 0;
						for (B = 0; B < E; ++B) {
							if (I = {
								child: []
							}, z = a(K[d].rows[B]), x = [], z.hasClass(f.cssChildRow) && 0 !== B) {
								D = H.normalized.length - 1, H.normalized[D][f.columns].$row = H.normalized[D][f.columns].$row.add(z), z.prev().hasClass(f.cssChildRow) || z.prev().addClass(R.css.cssHasChild), I.child[D] = a.trim(z[0].textContent || z[0].innerText || z.text() || "")
							} else {
								I.$row = z;
								I.order = B;
								for (C = 0; C < f.columns; ++C) {
									"undefined" === typeof J[C] ? f.debug && S("No parser found for cell:", z[0].cells[C], "does it have a header?") : (D = Q(G, z[0].cells[C], C), D = "no-parser" === J[C].id ? "" : J[C].format(D, G, z[0].cells[C], C), x.push(D), "numeric" === (J[C].type || "").toLowerCase() && (n[C] = Math.max(Math.abs(D) || 0, n[C] || 0)))
								}
								x[f.columns] = I;
								H.normalized.push(x)
							}
						...
						H.colMax = n;
						f.totalRows += H.normalized.length
					}
				}
				f.showProcessing && R.isProcessing(G);
				f.debug && s("Building cache for " + E + " rows", v)
			}
			function k(H, I) {
				var E = H.config,
					B = E.widgetOptions,
					...
					D, x, G, J, v, p;
				if (P(z)) {
					return E.appender ? E.appender(H, f) : H.isUpdating ? E.$table.trigger("updateComplete", H) : ""
				}
				E.debug && (p = new Date);
				for (v = 0; v < C.length; v++) {
					if (D = a(C[v]), D.length && !D.hasClass(E.cssInfoBlock)) {
						G = R.processTbody(H, D, !0);
						D = z[v].normalized;
						x = D.length;
						for (J = 0; J < x; J++) {
							f.push(D[J][E.columns].$row), E.appender && (!E.pager || E.pager.removeRows && B.pager_removeRows || E.pager.ajax) || G.append(D[J][E.columns].$row)
						}
						R.processTbody(H, G, !1)
					}
				}
				E.appender && E.appender(H, f);
				E.debug && s("Rebuilt table", p);
				I || E.appender || R.applyWidget(H);
				H.isUpdating && E.$table.trigger("updateComplete", H)
			}
			function A(d) {
				return /^d/i.test(d) || 1 === d
			}
			function y(B) {
				var C, z, v, x, d, p, n, f = B.config;
				f.headerList = [];
				f.headerContent = [];
				f.debug && (n = new Date);
				f.columns = R.computeColumnIndex(f.$table.children("thead, tfoot").children("tr"));
				x = f.cssIcon ? '<i class="' + (f.cssIcon === R.css.icon ? R.css.icon : f.cssIcon + " " + R.css.icon) + '"></i>' : "";
				f.$headers.each(function(D) {
					z = a(this);
					C = R.getColumnData(B, f.headers, D, !0);
					f.headerContent[D] = a(this).html();
					d = f.headerTemplate.replace(/\{content\}/g, a(this).html()).replace(/\{icon\}/g, x);
					f.onRenderTemplate && (v = f.onRenderTemplate.apply(z, [D, d])) && "string" === typeof v && (d = v);
					a(this).html('<div class="' + R.css.headerIn + '">' + d + "</div>");
					f.onRenderHeader && f.onRenderHeader.apply(z, [D]);
					this.column = parseInt(a(this).attr("data-column"), 10);
					this.order = A(R.getData(z, C, "sortInitialOrder") || f.sortInitialOrder) ? [1, 0, 2] : [0, 1, 2];
					this.count = -1;
					this.lockedOrder = !1;
					p = R.getData(z, C, "lockedOrder") || !1;
					"undefined" !== typeof p && !1 !== p && (this.order = this.lockedOrder = A(p) ? [1, 1, 1] : [0, 0, 0]);
					z.addClass(R.css.header + " " + f.cssHeader);
					f.headerList[D] = this;
					z.parent().addClass(R.css.headerRow + " " + f.cssHeaderRow).attr("role", "row");
					f.tabIndex && z.attr("tabindex", 0)
				}).attr({
					scope: "col",
					role: "columnheader"
				});
				F(B);
				f.debug && (s("Built headers:", n), S(f.$headers))
			}
			function u(d, f, p) {
				var n = d.config;
				n.$table.find(n.selectorRemove).remove();
				w(d);
				o(d);
				q(n.$table, f, p)
			}
			function F(d) {
				var f, p, n = d.config;
				n.$headers.each(function(x, v) {
					p = a(v);
					f = "false" === R.getData(v, R.getColumnData(d, n.headers, x, !0), "sorter");
					v.sortDisabled = f;
					p[f ? "addClass" : "removeClass"]("sorter-false").attr("aria-disabled", "" + f);
					d.id && (f ? p.removeAttr("aria-controls") : p.attr("aria-controls", d.id))
				})
			}
			function r(D) {
				var E, B, v = D.config,
					x = v.sortList,
					f = x.length,
					z = R.css.sortNone + " " + v.cssNone,
					p = [R.css.sortAsc + " " + v.cssAsc, R.css.sortDesc + " " + v.cssDesc],
					n = ["ascending", "descending"],
					C = a(D).find("tfoot tr").children().add(v.$extraHeaders).removeClass(p.join(" "));
				v.$headers.removeClass(p.join(" ")).addClass(z).attr("aria-sort", "none");
				for (E = 0; E < f; E++) {
					if (2 !== x[E][1] && (D = v.$headers.not(".sorter-false").filter('[data-column="' + x[E][0] + '"]' + (1 === f ? ":last" : "")), D.length)) {
						for (B = 0; B < D.length; B++) {
							D[B].sortDisabled || D.eq(B).removeClass(z).addClass(p[x[E][1]]).attr("aria-sort", n[x[E][1]])
						}
						C.length && C.filter('[data-column="' + x[E][0] + '"]').removeClass(z).addClass(p[x[E][1]])
					}
				}
				v.$headers.not(".sorter-false").each(function() {
					var d = a(this),
						G = this.order[(this.count + 1) % (v.sortReset ? 3 : 2)],
						G = d.text() + ": " + R.language[d.hasClass(R.css.sortAsc) ? "sortAsc" : d.hasClass(R.css.sortDesc) ? "sortDesc" : "sortNone"] + R.language[0 === G ? "nextAsc" : 1 === G ? "nextDesc" : "nextNone"];
					d.attr("aria-label", G)
				})
			}
			function h(d) {
				if (d.config.widthFixed && 0 === a(d).find("colgroup").length) {
					var f = a("<colgroup>"),
						n = a(d).width();
					a(d.tBodies[0]).find("tr:first").children("td:visible").each(function() {
						f.append(a("<col>").css("width", parseInt(a(this).width() / n * 1000, 10) / 10 + "%"))
					});
					a(d).prepend(f)
				}
			}
			function g(D, E) {
				var C, v, z, x, B, p = D.config,
					n = E || p.sortList;
				p.sortList = [];
				a.each(n, function(d, f) {
					x = parseInt(f[0], 10);
					if (z = p.$headers.filter('[data-column="' + x + '"]:last')[0]) {
						v = (v = ("" + f[1]).match(/^(1|d|s|o|n)/)) ? v[0] : "";
						switch (v) {
						case "1":
						case "d":
							v = 1;
						...
						case "s":
							...
							break;
						case "o":
							C = z.order[(B || 0) % (p.sortReset ? 3 : 2)];
							v = 0 === C ? 1 : 1 === C ? 0 : 2;
							break;
						case "n":
							z.count += 1;
							v = z.order[z.count % (p.sortReset ? 3 : 2)];
							break;
						default:
							v = 0
						}
						...
						C = [x, parseInt(v, 10) || 0];
						p.sortList.push(C);
						v = a.inArray(C[1], z.order);
						z.count = 0 <= v ? v : C[1] % (p.sortReset ? 3 : 2)
					}
				})
			}
			function e(d, f) {
				return d && d[f] ? d[f].type || "" : ""
			}
			function c(C, D, B) {
				var v, x, z, p = C.config,
					n = !B[p.sortMultiSortKey],
					f = p.$table;
				f.trigger("sortStart", C);
				D.count = B[p.sortResetKey] ? 2 : (D.count + 1) % (p.sortReset ? 3 : 2);
				p.sortRestart && (x = D, p.$headers.each(function() {
					this === x || !n && a(this).is("." + R.css.sortDesc + ",." + R.css.sortAsc) || (this.count = -1)
				}));
				x = D.column;
				if (n) {
					p.sortList = [];
					if (null !== p.sortForce) {
						for (v = p.sortForce, B = 0; B < v.length; B++) {
							v[B][0] !== x && p.sortList.push(v[B])
						}
					}
					v = D.order[D.count];
					if (2 > v && (p.sortList.push([x, v]), 1 < D.colSpan)) {
						for (B = 1; B < D.colSpan; B++) {
							p.sortList.push([x + B, v])
						}
					}
				} else {
					if (p.sortAppend && 1 < p.sortList.length) {
						for (B = 0; B < p.sortAppend.length; B++) {
							z = R.isValueInArray(p.sortAppend[B][0], p.sortList), 0 <= z && p.sortList.splice(z, 1)
						}
					}
					if (0 <= R.isValueInArray(x, p.sortList)) {
						for (B = 0; B < p.sortList.length; B++) {
							z = p.sortList[B], v = p.$headers.filter('[data-column="' + z[0] + '"]:last')[0], z[0] === x && (z[1] = v.order[D.count], 2 === z[1] && (p.sortList.splice(B, 1), v.count = -1))
						}
					} else {
						if (v = D.order[D.count], 2 > v && (p.sortList.push([x, v]), 1 < D.colSpan)) {
							for (B = 1; B < D.colSpan; B++) {
								p.sortList.push([x + B, v])
							}
						}
				...
				if (null !== p.sortAppend) {
					for (v = p.sortAppend, B = 0; B < v.length; B++) {
						v[B][0] !== x && p.sortList.push(v[B])
					}
				}
				f.trigger("sortBegin", C);
				setTimeout(function() {
					r(C);
					m(C);
					k(C);
			...
			function m(U) {
				var V, T, L, N, O, K, M, J, p, I, D, H = 0,
					E = U.config,
					C = E.textSorter || "",
					v = E.sortList,
					f = v.length,
					G = U.tBodies.length;
				if (!E.serverSideSorting && !P(E.cache)) {
					E.debug && (O = new Date);
					for (T = 0; T < G; T++) {
						K = E.cache[T].colMax, M = E.cache[T].normalized, M.sort(function(x, n) {
							for (V = 0; V < f; V++) {
								N = v[V][0];
								J = v[V][1];
								H = 0 === J;
								if (E.sortStable && x[N] === n[N] && 1 === f) {
									break
								}(L = /n/i.test(e(E.parsers, N))) && E.strings[N] ? (L = "boolean" === typeof E.string[E.strings[N]] ? (H ? 1 : -1) * (E.string[E.strings[N]] ? -1 : 1) : E.strings[N] ? E.string[E.strings[N]] || 0 : 0, p = E.numberSorter ? E.numberSorter(x[N], n[N], H, K[N], U) : R["sortNumeric" + (H ? "Asc" : "Desc")](x[N], n[N], L, K[N], N, U)) : (I = H ? x : n, D = H ? n : x, p = "function" === typeof C ? C(I[N], D[N], H, N, U) : "object" === typeof C && C.hasOwnProperty(N) ? C[N](I[N], D[N], H, N, U) : R["sortNatural" + (H ? "Asc" : "Desc")](x[N], n[N], N, U, E));
								if (p) {
									return p
								}
							}
							return x[E.columns].order - n[E.columns].order
						})
					}
					E.debug && s("Sorting on " + v.toString() + " and dir " + J + " time", O)
				}
			}
			function l(d, f) {
				d[0].isUpdating && d.trigger("updateComplete");
				a.isFunction(f) && f(d[0])
			}
			function q(d, f, p) {
				var n = d[0].config.sortList;
				!1 !== f && !d[0].isProcessing && n.length ? d.trigger("sorton", [n, function() {
					l(d, p)
				}, !0]) : (l(d, p), R.applyWidget(d[0], !1))
			}
			function j(d) {
				var f = d.config,
					n = f.$table;
				n.unbind("sortReset update updateRows updateCell updateAll addRows updateComplete sorton appendCache updateCache applyWidgetId applyWidgets refreshWidgets destroy mouseup mouseleave ".split(" ").join(f.namespace + " ")).bind("sortReset" + f.namespace, function(v, p) {
					v.stopPropagation();
					f.sortList = [];
					r(d);
					m(d);
					k(d);
					a.isFunction(p) && p(d)
				}).bind("updateAll" + f.namespace, function(x, p, v) {
					x.stopPropagation();
					d.isUpdating = !0;
					R.refreshWidgets(d, !0, !0);
					R.restoreHeaders(d);
					y(d);
					R.bindEvents(d, f.$headers, !0);
					j(d);
					u(d, p, v)
				}).bind("update" + f.namespace + " updateRows" + f.namespace, function(p, x, v) {
					p.stopPropagation();
					d.isUpdating = !0;
					F(d);
					u(d, x, v)
				}).bind("updateCell" + f.namespace, function(z, D, E, B) {
					z.stopPropagation();
					d.isUpdating = !0;
					n.find(f.selectorRemove).remove();
					var x, v;
					x = n.find("tbody");
					v = a(D);
					z = x.index(a.fn.closest ? v.closest("tbody") : v.parents("tbody").filter(":first"));
					var C = a.fn.closest ? v.closest("tr") : v.parents("tr").filter(":first");
					D = v[0];
					x.length && 0 <= z && (x = x.eq(z).find("tr").index(C), v = v.index(), f.cache[z].normalized[x][f.columns].$row = C, D = f.cache[z].normalized[x][v] = "no-parser" === f.parsers[v].id ? "" : f.parsers[v].format(Q(d, D, v), d, D, v), "numeric" === (f.parsers[v].type || "").toLowerCase() && (f.cache[z].colMax[v] = Math.max(Math.abs(D) || 0, f.cache[z].colMax[v] || 0)), q(n, E, B))
				}).bind("addRows" + f.namespace, function(C, E, G, D) {
					C.stopPropagation();
					d.isUpdating = !0;
					if (P(f.cache)) {
						F(d), u(d, G, D)
						...
						E = a(E);
						var B, z, I, J, H = E.filter("tr").length,
							p = n.find("tbody").index(E.parents("tbody").filter(":first"));
						f.parsers && f.parsers.length || w(d);
						for (C = 0; C < H; C++) {
							z = E[C].cells.length;
							J = [];
							I = {
								child: [],
								$row: E.eq(C),
								order: f.cache[p].normalized.length
							};
							for (B = 0; B < z; B++) {
								J[B] = "no-parser" === f.parsers[B].id ? "" : f.parsers[B].format(Q(d, E[C].cells[B], B), d, E[C].cells[B], B), "numeric" === (f.parsers[B].type || "").toLowerCase() && (f.cache[p].colMax[B] = Math.max(Math.abs(J[B]) || 0, f.cache[p].colMax[B] || 0))
							}
							J.push(I);
							f.cache[p].normalized.push(J)
						}
						q(n, G, D)
					}
				}).bind("updateComplete" + f.namespace, function() {
					d.isUpdating = !1
				}).bind("sorton" + f.namespace, function(v, z, B, x) {
					var p = d.config;
					v.stopPropagation();
					n.trigger("sortStart", this);
					g(d, z);
					r(d);
					p.delayInit && P(p.cache) && o(d);
					n.trigger("sortBegin", this);
					m(d);
					k(d, x);
					n.trigger("sortEnd", this);
					R.applyWidget(d);
					a.isFunction(B) && B(d)
				}).bind("appendCache" + f.namespace, function(p, x, v) {
					p.stopPropagation();
					k(d, v);
					a.isFunction(x) && x(d)
				}).bind("updateCache" + f.namespace, function(v, p) {
					f.parsers && f.parsers.length || w(d);
					o(d);
					a.isFunction(p) && p(d)
				}).bind("applyWidgetId" + f.namespace, function(v, p) {
					v.stopPropagation();
					R.getWidgetById(p).format(d, f, f.widgetOptions)
				}).bind("applyWidgets" + f.namespace, function(p, v) {
					p.stopPropagation();
					R.applyWidget(d, v)
				}).bind("refreshWidgets" + f.namespace, function(p, x, v) {
					p.stopPropagation();
					R.refreshWidgets(d, x, v)
				}).bind("destroy" + f.namespace, function(p, x, v) {
					p.stopPropagation();
					R.destroy(d, x, v)
				}).bind("resetToLoadState" + f.namespace, function() {
					R.refreshWidgets(d, !0, !0);
					f = a.extend(!0, R.defaults, f.originalSettings);
					d.hasInitialized = !1;
					R.setup(d, f)
				})
			}
			...
			R.log = S;
			R.benchmark = s;
			R.construct = function(d) {
				return this.each(function() {
					var f = a.extend(!0, {}, R.defaults, d);
					f.originalSettings = d;
					!this.hasInitialized && R.buildTable && "TABLE" !== this.tagName ? R.buildTable(this, f) : R.setup(this, f)
				})
			};
			R.setup = function(d, f) {
				if (!d || !d.tHead || 0 === d.tBodies.length || !0 === d.hasInitialized) {
					return f.debug ? S("ERROR: stopping initialization! No table, thead, tbody or tablesorter has already been initialized") : ""
				}
				var v = "",
					n = a(d),
					p = a.metadata;
				...
				d.isProcessing = !0;
				d.config = f;
				a.data(d, "tablesorter", f);
				f.debug && a.data(d, "startoveralltimer", new Date);
				f.supportsDataObject = function(x) {
					x[0] = parseInt(x[0], 10);
					return 1 < x[0] || 1 === x[0] && 4 <= parseInt(x[1], 10)
				}(a.fn.jquery.split("."));
				f.string = {
					max: 1,
					min: -1,
				...
				/tablesorter\-/.test(n.attr("class")) || (v = "" !== f.theme ? " tablesorter-" + f.theme : "");
				f.table = d;
				f.$table = n.addClass(R.css.table + " " + f.tableClass + v).attr({
					role: "grid"
				});
				f.$headers = a(d).find(f.selectorHeaders);
				f.namespace = f.namespace ? "." + f.namespace.replace(/\W/g, "") : ".tablesorter" + Math.random().toString(16).slice(2);
				f.$tbodies = n.children("tbody:not(." + f.cssInfoBlock + ")").attr({
					"aria-live": "polite",
					"aria-relevant": "all"
				});
				f.$table.find("caption").length && f.$table.attr("aria-labelledby", "theCaption");
				f.widgetInit = {};
				f.textExtraction = f.$table.attr("data-text-extraction") || f.textExtraction || "basic";
				y(d);
				h(d);
				w(d);
				f.totalRows = 0;
				...
				R.bindEvents(d, f.$headers, !0);
				j(d);
				f.supportsDataObject && "undefined" !== typeof n.data().sortlist ? f.sortList = n.data().sortlist : p && n.metadata() && n.metadata().sortlist && (f.sortList = n.metadata().sortlist);
				R.applyWidget(d, !0);
				0 < f.sortList.length ? n.trigger("sorton", [f.sortList,
				{}, !f.initWidgets, !0]) : (r(d), f.initWidgets && R.applyWidget(d, !1));
				f.showProcessing && n.unbind("sortBegin" + f.namespace + " sortEnd" + f.namespace).bind("sortBegin" + f.namespace + " sortEnd" + f.namespace, function(x) {
					clearTimeout(f.processTimer);
					R.isProcessing(d);
					"sortBegin" === x.type && (f.processTimer = setTimeout(function() {
						R.isProcessing(d, !0)
					}, 500))
				});
				d.hasInitialized = !0;
				d.isProcessing = !1;
				f.debug && R.benchmark("Overall initialization time", a.data(d, "startoveralltimer"));
				n.trigger("tablesorter-initialized", d);
				"function" === typeof f.initialized && f.initialized(d)
			};
			R.getColumnData = function(f, n, z, p) {
				if ("undefined" !== typeof n && null !== n) {
					f = a(f)[0];
					var v, x = f.config;
					if (n[z]) {
						return p ? n[z] : n[x.$headers.index(x.$headers.filter('[data-column="' + z + '"]:last'))]
					}
					for (v in n) {
						if ("string" === typeof v && (f = p ? x.$headers.eq(z).filter(v) : x.$headers.filter('[data-column="' + z + '"]:last').filter(v), f.length)) {
							return n[v]
						}
					}
				}
			};
			R.computeColumnIndex = function(K) {
				var L = [],
					J = 0,
					E, H, I, G, D, C, z, M, B, x;
				for (E = 0; E < K.length; E++) {
					for (D = K[E].cells, H = 0; H < D.length; H++) {
						I = D[H];
						G = a(I);
						C = I.parentNode.rowIndex;
						G.index();
						z = I.rowSpan || 1;
						M = I.colSpan || 1;
						"undefined" === typeof L[C] && (L[C] = []);
						for (I = 0; I < L[C].length + 1; I++) {
							if ("undefined" === typeof L[C][I]) {
								B = I;
								break
							}
						}
						...
						for (I = C; I < C + z; I++) {
							for ("undefined" === typeof L[I] && (L[I] = []), x = L[I], G = B; G < B + M; G++) {
								x[G] = "x"
							}
						}
					}
			...
			R.isProcessing = function(f, n, x) {
				f = a(f);
				var v = f[0].config,
					p = x || f.find("." + R.css.header);
				n ? ("undefined" !== typeof x && 0 < v.sortList.length && (p = p.filter(function() {
					return this.sortDisabled ? !1 : 0 <= R.isValueInArray(parseFloat(a(this).attr("data-column")), v.sortList)
				})), f.add(p).addClass(R.css.processing + " " + v.cssProcessing)) : f.add(p).removeClass(R.css.processing + " " + v.cssProcessing)
			};
			R.processTbody = function(d, f, n) {
				d = a(d)[0];
				if (n) {
					return d.isProcessing = !0, f.before('<span class="tablesorter-savemyplace"/>'), n = a.fn.detach ? f.detach() : f.remove()
				}
				n = a(d).find("span.tablesorter-savemyplace");
				f.insertAfter(n);
				n.remove();
				d.isProcessing = !1
			};
			R.clearTableBody = function(d) {
				a(d)[0].config.$tbodies.children().detach()
			};
			R.bindEvents = function(f, n, x) {
				f = a(f)[0];
				var v, p = f.config;
				!0 !== x && (p.$extraHeaders = p.$extraHeaders ? p.$extraHeaders.add(n) : n);
				n.find(p.selectorSort).add(n.filter(p.selectorSort)).unbind(["mousedown", "mouseup", "sort", "keyup", ""].join(p.namespace + " ")).bind(["mousedown", "mouseup", "sort", "keyup", ""].join(p.namespace + " "), function(B, z) {
					var d;
					d = B.type;
					if (!(1 !== (B.which || B.button) && !/sort|keyup/.test(d) || "keyup" === d && 13 !== B.which || "mouseup" === d && !0 !== z && 250 < (new Date).getTime() - v)) {
						if ("mousedown" === d) {
							return v = (new Date).getTime(), /(input|select|button|textarea)/i.test(B.target.tagName) ? "" : !p.cancelSelection
						}
						p.delayInit && P(p.cache) && o(f);
						d = a.fn.closest ? a(this).closest("th, td")[0] : /TH|TD/.test(this.tagName) ? this : a(this).parents("th, td")[0];
						d = p.$headers[n.index(d)];
						d.sortDisabled || c(f, d, B)
					}
				});
				p.cancelSelection && n.attr("unselectable", "on").bind("selectstart", !1).css({
					"user-select": "none",
					MozUserSelect: "none"
				})
			};
			R.restoreHeaders = function(d) {
				var f = a(d)[0].config;
				f.$table.find(f.selectorHeaders).each(function(n) {
					a(this).find("." + R.css.headerIn).length && a(this).html(f.headerContent[n])
				})
			};
			R.destroy = function(f, p, C) {
				f = a(f)[0];
				if (f.hasInitialized) {
					R.refreshWidgets(f, !0, !0);
					var B = a(f),
						z = f.config,
						x = B.find("thead:first"),
						v = x.find("tr." + R.css.headerRow).removeClass(R.css.headerRow + " " + z.cssHeaderRow),
						n = B.find("tfoot:first > tr").children("th, td");
					!1 === p && 0 <= a.inArray("uitheme", z.widgets) && (B.trigger("applyWidgetId", ["uitheme"]), B.trigger("applyWidgetId", ["zebra"]));
					x.find("tr").not(v).remove();
					B.removeData("tablesorter").unbind("sortReset update updateAll updateRows updateCell addRows updateComplete sorton appendCache updateCache applyWidgetId applyWidgets refreshWidgets destroy mouseup mouseleave keypress sortBegin sortEnd resetToLoadState ".split(" ").join(z.namespace + " "));
					z.$headers.add(n).removeClass([R.css.header, z.cssHeader, z.cssAsc, z.cssDesc, R.css.sortAsc, R.css.sortDesc, R.css.sortNone].join(" ")).removeAttr("data-column").removeAttr("aria-label").attr("aria-disabled", "true");
					v.find(z.selectorSort).unbind(["mousedown", "mouseup", "keypress", ""].join(z.namespace + " "));
					R.restoreHeaders(f);
					B.toggleClass(R.css.table + " " + z.tableClass + " tablesorter-" + z.theme, !1 === p);
					f.hasInitialized = !1;
					delete f.config.cache;
			...
			R.sortNatural = function(f, p) {
				...
				B = R.regex;
				if (B.hex.test(p)) {
					C = parseInt(f.match(B.hex), 16);
					z = parseInt(p.match(B.hex), 16);
					if (C < z) {
						return -1
					}
					if (C > z) {
				...
				C = f.replace(B.chunk, "\\0$1\\0").replace(B.chunks, "").split("\\0");
				B = p.replace(B.chunk, "\\0$1\\0").replace(B.chunks, "").split("\\0");
				n = Math.max(C.length, B.length);
				for (v = 0; v < n; v++) {
					z = isNaN(C[v]) ? C[v] || 0 : parseFloat(C[v]) || 0;
					x = isNaN(B[v]) ? B[v] || 0 : parseFloat(B[v]) || 0;
					if (isNaN(z) !== isNaN(x)) {
						return isNaN(z) ? 1 : -1
					}
					typeof z !== typeof x && (z += "", x += "");
					if (z < x) {
						return -1
					}
					if (z > x) {
			...
			R.sortNaturalAsc = function(f, n, x, v, p) {
				if (f === n) {
					return 0
				}
				x = p.string[p.empties[x] || p.emptyTo];
				return "" === f && 0 !== x ? "boolean" === typeof x ? x ? -1 : 1 : -x || -1 : "" === n && 0 !== x ? "boolean" === typeof x ? x ? 1 : -1 : x || 1 : R.sortNatural(f, n)
			};
			R.sortNaturalDesc = function(f, n, x, v, p) {
				if (f === n) {
					return 0
				}
				x = p.string[p.empties[x] || p.emptyTo];
				return "" === f && 0 !== x ? "boolean" === typeof x ? x ? -1 : 1 : x || 1 : "" === n && 0 !== x ? "boolean" === typeof x ? x ? 1 : -1 : -x || -1 : R.sortNatural(n, f)
			};
			R.sortText = function(d, f) {
				return d > f ? 1 : d < f ? -1 : 0
			};
			R.getTextValue = function(f, n, x) {
				if (x) {
					var v = f ? f.length : 0,
						p = x + n;
					for (x = 0; x < v; x++) {
			...
			R.sortNumericAsc = function(f, n, z, x, v, p) {
				if (f === n) {
					return 0
				}
				p = p.config;
				v = p.string[p.empties[v] || p.emptyTo];
				if ("" === f && 0 !== v) {
					return "boolean" === typeof v ? v ? -1 : 1 : -v || -1
				}
				if ("" === n && 0 !== v) {
					return "boolean" === typeof v ? v ? 1 : -1 : v || 1
				}
				isNaN(f) && (f = R.getTextValue(f, z, x));
				isNaN(n) && (n = R.getTextValue(n, z, x));
				return f - n
			};
			R.sortNumericDesc = function(f, n, z, x, v, p) {
				if (f === n) {
					return 0
				}
				p = p.config;
			...
			R.sortNumeric = function(d, f) {
				return d - f
			};
			R.characterEquivalents = {
				a: "\u00e1\u00e0\u00e2\u00e3\u00e4\u0105\u00e5",
				A: "\u00c1\u00c0\u00c2\u00c3\u00c4\u0104\u00c5",
				c: "\u00e7\u0107\u010d",
				C: "\u00c7\u0106\u010c",
			...
			R.replaceAccents = function(f) {
				var n, v = "[",
					p = R.characterEquivalents;
				if (!R.characterRegex) {
					R.characterRegexArray = {};
					for (n in p) {
						"string" === typeof n && (v += p[n], R.characterRegexArray[n] = new RegExp("[" + p[n] + "]", "g"))
					}
					R.characterRegex = new RegExp(v + "]")
				}
				if (R.characterRegex.test(f)) {
					for (n in p) {
						"string" === typeof n && (f = f.replace(R.characterRegexArray[n], n))
					}
				}
				return f
			};
			R.isValueInArray = function(f, n) {
				var v, p = n.length;
				for (v = 0; v < p; v++) {
					if (n[v][0] === f) {
						return v
			...
			R.addParser = function(f) {
				var n, v = R.parsers.length,
					p = !0;
				for (n = 0; n < v; n++) {
					R.parsers[n].id.toLowerCase() === f.id.toLowerCase() && (p = !1)
				}
				p && R.parsers.push(f)
			};
			R.getParserById = function(d) {
				if ("false" == d) {
					return !1
				}
				var f, n = R.parsers.length;
				for (f = 0; f < n; f++) {
					if (R.parsers[f].id.toLowerCase() === d.toString().toLowerCase()) {
						return R.parsers[f]
					}
				}
				return !1
			};
			R.addWidget = function(d) {
				R.widgets.push(d)
			};
			R.hasWidget = function(d, f) {
				d = a(d);
				return d.length && d[0].config && d[0].config.widgetInit[f] || !1
			};
			R.getWidgetById = function(f) {
				var n, v, p = R.widgets.length;
				for (n = 0; n < p; n++) {
					if ((v = R.widgets[n]) && v.hasOwnProperty("id") && v.id.toLowerCase() === f.toLowerCase()) {
						return v
					}
				}
			};
			R.applyWidget = function(f, v) {
				f = a(f)[0];
				var D = f.config,
					C = D.widgetOptions,
					B = [],
					z, x, n;
				!1 !== v && f.hasInitialized && (f.isApplyingWidgets || f.isUpdating) || (D.debug && (z = new Date), D.widgets.length && (f.isApplyingWidgets = !0, D.widgets = a.grep(D.widgets, function(p, d) {
					return a.inArray(p, D.widgets) === d
				}), a.each(D.widgets || [], function(p, d) {
					(n = R.getWidgetById(d)) && n.id && (n.priority || (n.priority = 10), B[p] = n)
				}), B.sort(function(p, d) {
					return p.priority < d.priority ? -1 : p.priority === d.priority ? 0 : 1
				}), a.each(B, function(p, d) {
					if (d) {
						if (v || !D.widgetInit[d.id]) {
							D.widgetInit[d.id] = !0, d.hasOwnProperty("options") && (C = f.config.widgetOptions = a.extend(!0, {}, d.options, C)), d.hasOwnProperty("init") && d.init(f, d, D, C)
						}!v && d.hasOwnProperty("format") && d.format(f, D, C, !1)
					}
				})), setTimeout(function() {
					f.isApplyingWidgets = !1
				}, 0), D.debug && (x = D.widgets.length, s("Completed " + (!0 === v ? "initializing " : "applying ") + x + " widget" + (1 !== x ? "s" : ""), z)))
			};
			R.refreshWidgets = function(d, n, C) {
				d = a(d)[0];
				var x, B = d.config,
					z = B.widgets,
					v = R.widgets,
					f = v.length;
				for (x = 0; x < f; x++) {
					v[x] && v[x].id && (n || 0 > a.inArray(v[x].id, z)) && (B.debug && S('Refeshing widgets: Removing "' + v[x].id + '"'), v[x].hasOwnProperty("remove") && B.widgetInit[v[x].id] && (v[x].remove(d, B, B.widgetOptions), B.widgetInit[v[x].id] = !1))
				}!0 !== C && R.applyWidget(d, n)
			};
			R.getData = function(n, p, B) {
				var z = "";
				n = a(n);
				var x, v;
				if (!n.length) {
				...
				x = a.metadata ? n.metadata() : !1;
				v = " " + (n.attr("class") || "");
				"undefined" !== typeof n.data(B) || "undefined" !== typeof n.data(B.toLowerCase()) ? z += n.data(B) || n.data(B.toLowerCase()) : x && "undefined" !== typeof x[B] ? z += x[B] : p && "undefined" !== typeof p[B] ? z += p[B] : " " !== v && v.match(" " + B + "-") && (z = v.match(new RegExp("\\s" + B + "-([\\w-]+)"))[1] || "");
				return a.trim(z)
			};
			R.formatFloat = function(d, f) {
				if ("string" !== typeof d || "" === d) {
					return d
				}
				var n;
				d = (f && f.config ? !1 !== f.config.usNumberFormat : "undefined" !== typeof f ? f : 1) ? d.replace(/,/g, "") : d.replace(/[\s|\.]/g, "").replace(/,/g, ".");
				/^\s*\([.\d]+\)/.test(d) && (d = d.replace(/^\s*\(([.\d]+)\)/, "-$1"));
				n = parseFloat(d);
				return isNaN(n) ? a.trim(d) : n
			};
			R.isDigit = function(d) {
	...
	b.addParser({
		...
		format: function(f, c) {
			var e = c.config;
			f && (f = a.trim(e.ignoreCase ? f.toLocaleLowerCase() : f), f = e.sortLocaleCompare ? b.replaceAccents(f) : f);
			return f
		},
		type: "text"
	});
	b.addParser({
		...
		format: function(f, c) {
			var e = b.formatFloat((f || "").replace(/[^\w,. \-()]/g, ""), c);
			return f && "number" === typeof e ? e : f ? a.trim(f && c.config.ignoreCase ? f.toLocaleLowerCase() : f) : f
		},
		type: "numeric"
	});
	b.addParser({
	...
	b.addParser({
		...
		format: function(c) {
			return c ? a.trim(c.replace(/(https?|ftp|file):\/\//, "")) : c
		},
		type: "text"
	});
	b.addParser({
		...
		format: function(e, c) {
			return e ? b.formatFloat("" !== e ? (new Date(e.replace(/-/g, "/"))).getTime() || e : "", c) : e
		},
		type: "numeric"
	});
	b.addParser({
		...
		format: function(e, c) {
			return e ? b.formatFloat(e.replace(/%/g, ""), c) : e
		},
		type: "numeric"
	});
	b.addParser({
		...
		format: function(e, c) {
			return e ? b.formatFloat((new Date(e.replace(/(\S)([AP]M)$/i, "$1 $2"))).getTime() || e, c) : e
		},
		type: "numeric"
	});
	b.addParser({
		...
		format: function(j, f, h, c) {
			if (j) {
				h = f.config;
				var e = h.$headers.filter("[data-column=" + c + "]:last");
				c = e.length && e[0].dateFormat || b.getData(e, b.getColumnData(f, h.headers, c), "dateFormat") || h.dateFormat;
				j = j.replace(/\s+/g, " ").replace(/[\-.,]/g, "/");
				"mmddyyyy" === c ? j = j.replace(/(\d{1,2})[\/\s](\d{1,2})[\/\s](\d{4})/, "$3/$1/$2") : "ddmmyyyy" === c ? j = j.replace(/(\d{1,2})[\/\s](\d{1,2})[\/\s](\d{4})/, "$3/$2/$1") : "yyyymmdd" === c && (j = j.replace(/(\d{4})[\/\s](\d{1,2})[\/\s](\d{1,2})/, "$1/$2/$3"))
			}
			return j ? b.formatFloat((new Date(j)).getTime() || j, f) : j
		},
		type: "numeric"
	});
	b.addParser({
		...
		format: function(e, c) {
			return e ? b.formatFloat((new Date("2000/01/01 " + e.replace(/(\S)([AP]M)$/i, "$1 $2"))).getTime() || e, c) : e
		},
		type: "numeric"
	});
	b.addParser({
		...
		format: function(e, f, c) {
			e = f.config;
			e = e.parserMetadataName ? e.parserMetadataName : "sortValue";
			return a(c).metadata()[e]
		},
		type: "numeric"
	});
	b.addWidget({
		id: "zebra",
		priority: 90,
		format: function(h, m, g) {
			var l, n, k, j, e, c, o = new RegExp(m.cssChildRow, "i"),
				f = m.$tbodies;
			m.debug && (e = new Date);
			for (h = 0; h < f.length; h++) {
				l = f.eq(h), c = l.children("tr").length, 1 < c && (k = 0, l = l.children("tr:visible").not(m.selectorRemove), l.each(function() {
					n = a(this);
					o.test(this.className) || k++;
					j = 0 === k % 2;
					n.removeClass(g.zebra[j ? 1 : 0]).addClass(g.zebra[j ? 0 : 1])
				}))
			}
			m.debug && b.benchmark("Applying Zebra widget", e)
		},
		remove: function(g, h, f) {
			var c;
			h = h.$tbodies;
			var e = (f.zebra || ["even", "odd"]).join(" ");
			for (f = 0; f < h.length; f++) {
				c = a.tablesorter.processTbody(g, h.eq(f), !0), c.children().removeClass(e), a.tablesorter.processTbody(g, c, !1)
			}
		}
	})
}(jQuery);
!
function(d) {
	var c = d.tablesorter = d.tablesorter || {};
	d.extend(c.css, {
		table: "tablesorter",
		header: "header",
		sortAsc: "headerSortUp",
		sortDesc: "headerSortDown"
	}), c.storage = function(C, B, A, z) {
		C = d(C)[0];
		var y, x, w, v = !1,
			u = {},
			s = C.config,
			r = d(C),
			q = z && z.id || r.attr(z && z.group || "data-table-group") || C.id || d(".tablesorter").index(r),
			p = z && z.url || r.attr(z && z.page || "data-table-page") || s && s.fixedUrl || window.location.pathname;
		if ("localStorage" in window) {
			try {
				window.localStorage.setItem("_tmptest", "temp"), v = !0, window.localStorage.removeItem("_tmptest")
			} catch (a) {}
		}
		return d.parseJSON && (v ? u = d.parseJSON(localStorage[B] || "{}") : (x = document.cookie.split(/[;\s|=]/), y = d.inArray(B, x) + 1, u = 0 !== y ? d.parseJSON(x[y] || "{}") : {})), (A || "" === A) && window.JSON && JSON.hasOwnProperty("stringify") ? (u[p] || (u[p] = {}), u[p][q] = A, v ? localStorage[B] = JSON.stringify(u) : (w = new Date, w.setTime(w.getTime() + 31536000000), document.cookie = B + "=" + JSON.stringify(u).replace(/\"/g, '"') + "; expires=" + w.toGMTString() + "; path=/"), void 0) : u && u[p] ? u[p][q] : ""
	}, c.addWidget({
		id: "saveSort",
		priority: 20,
		options: {
		...
		format: function(r, q, p, o) {
			var n, m, l = q.$table,
				b = p.saveSort !== !1,
				a = {
					sortList: q.sortList
				};
			q.debug && (m = new Date), l.hasClass("hasSaveSort") ? b && r.hasInitialized && c.storage && (c.storage(r, "tablesorter-savesort", a), q.debug && c.benchmark("saveSort widget: Saving last sort: " + q.sortList, m)) : (l.addClass("hasSaveSort"), a = "", c.storage && (n = c.storage(r, "tablesorter-savesort"), a = n && n.hasOwnProperty("sortList") && d.isArray(n.sortList) ? n.sortList : "", q.debug && c.benchmark('saveSort: Last sort loaded: "' + a + '"', m), l.bind("saveSortReset", function(e) {
				e.stopPropagation(), c.storage(r, "tablesorter-savesort", "")
			})), o && a && a.length > 0 ? q.sortList = a : r.hasInitialized && a && a.length > 0 && l.trigger("sorton", [a]))
		},
		remove: function(b) {
...
function(b, c, a) {
	...
	b.zui || (b.zui = function(g) {
		b.isPlainObject(g) && b.extend(b.zui, g)
	});
	var f = {
		all: -1,
		left: 0,
	...
	b.zui({
		uuid: function(g) {
			var h = 1000 * (new Date).getTime() + d++ % 1000;
			return g ? h : h.toString(36)
		},
		callEvent: function(h, k, j) {
			if (b.isFunction(h)) {
				j !== a && (h = b.proxy(h, j));
				var g = h(k);
				return k && (k.result = g), !(g !== a && !g)
			}
			return 1
		},
		clientLang: function() {
			...
			if ("undefined" != typeof h && h.clientLang && (e = h.clientLang), !e) {
				var g = b("html").attr("lang");
				e = g ? g : navigator.userLanguage || navigator.userLanguage || "zh_cn"
			}
		...
		getMouseButtonCode: function(e) {
			return "number" != typeof e && (e = f[e]), e !== a && null !== e || (e = -1), e
		}
	}), b.fn.callEvent = function(q, v, u) {
		var m = b(this),
			j = q.indexOf(".zui."),
			p = j < 0 ? q : q.substring(0, j),
			g = b.Event(p, v);
		if (u === a && j > 0 && (u = m.data(q.substring(j + 1))), u && u.options) {
			var k = u.options[p];
			b.isFunction(k) && (g.result = b.zui.callEvent(k, g, u))
		}
		return m.trigger(g), g
	}, b.fn.callComEvent = function(k, m, l) {
		l === a || b.isArray(l) || (l = [l]);
		var h = this,
			g = h.triggerHandler(m, l),
			j = k.options[m];
		return j && (g = j.apply(k, l)), g
	}
...
function(b) {
	...
	f.prototype.close = function(j) {
		function l() {
			g.trigger("closed." + a).remove()
		}
		var k = b(this),
			h = k.attr("data-target");
		h || (h = k.attr("href"), h = h && h.replace(/.*(?=#[^\s]*$)/, ""));
		var g = b(h);
		j && j.preventDefault(), g.length || (g = k.hasClass("alert") ? k : k.parent()), g.trigger(j = b.Event("close." + a)), j.isDefaultPrevented() || (g.removeClass("in"), b.support.transition && g.hasClass("fade") ? g.one(b.support.transition.end, l).emulateTransitionEnd(150) : l())
	};
	var d = b.fn.alert;
	b.fn.alert = function(g) {
		return this.each(function() {
			var h = b(this),
				e = h.data(a);
			e || h.data(a, e = new f(this)), "string" == typeof g && e[g].call(h)
		})
	}, b.fn.alert.Constructor = f, b.fn.alert.noConflict = function() {
		return b.fn.alert = d, this
	}, b(document).on("click." + a + ".data-api", c, f.prototype.close)
}(window.jQuery), function(b, d) {
	var a = "zui.pager",
		g = {
			page: 1,
			recTotal: 0,
		...
		c = function(k, l) {
			var h = this;
			h.name = a, h.$ = b(k), l = h.options = b.extend({}, c.DEFAULTS, this.$.data(), l);
			var j = l.lang || b.zui.clientLang();
			h.lang = b.isPlainObject(j) ? b.extend(!0, {}, f[j.lang || b.zui.clientLang()], j) : f[j], h.state = {}, h.set(l.page, l.recTotal, l.recPerPage, !0), h.$.on("click", ".pager-goto-btn", function() {
				var n = b(this).closest(".pager-goto"),
					m = parseInt(n.find(".pager-goto-input").val());
				NaN !== m && h.set(m)
			}).on("click", ".pager-item", function() {
				var m = b(this).data("page");
				"number" == typeof m && m > 0 && h.set(m)
			}).on("click", ".pager-size-menu [data-size]", function() {
				var m = b(this).data("size");
				"number" == typeof m && m > 0 && h.set(-1, -1, m)
			})
		};
	c.prototype.set = function(p, k, q, m) {
		var j = this;
		"object" == typeof p && null !== p && (q = p.recPerPage, k = p.recTotal, p = p.page);
		var n = j.state;
		n || (n = b.extend({}, g));
		var h = b.extend({}, n);
		return "number" == typeof q && q > 0 && (n.recPerPage = q), "number" == typeof k && k >= 0 && (n.recTotal = k), "number" == typeof p && p >= 0 && (n.page = p), n.totalPage = n.recTotal && n.recPerPage ? Math.ceil(n.recTotal / n.recPerPage) : 1, n.page = Math.max(0, Math.min(n.page, n.totalPage)), n.pageRecCount = n.recTotal, n.page && n.recTotal && (n.page < n.totalPage ? n.pageRecCount = n.recPerPage : n.page > 1 && (n.pageRecCount = n.recTotal - n.recPerPage * (n.page - 1))), n.skip = n.page > 1 ? (n.page - 1) * n.recPerPage : 0, n.start = n.skip + 1, n.end = n.skip + n.pageRecCount, n.prev = n.page > 1 ? n.page - 1 : 0, n.next = n.page < n.totalPage ? n.page + 1 : 0, j.state = n, m || h.page === n.page && h.recTotal === n.recTotal && h.recPerPage === n.recPerPage || j.$.callComEvent(j, "onPageChange", [n, h]), j.render()
	}, c.prototype.createLinkItem = function(h, l, k) {
		var j = this;
		l === d && (l = h);
		var e = b('<a title="' + j.lang.pageOfText.format(h) + '" class="pager-item" data-page="' + h + '"/>').attr("href", h ? j.createLink(h, j.state) : "###").html(l);
		return k || (e = b("<li />").append(e).toggleClass("active", h === j.state.page).toggleClass("disabled", !h || h === j.state.page)), e
	}, c.prototype.createNavItems = function(j) {
		var h = this,
			p = h.$,
			m = h.state,
			k = m.totalPage,
			...
			l = function(n, r) {
				if (n === !1) {
					return void p.append(h.createLinkItem(0, r || h.options.navEllipsisItem))
				}
				r === d && (r = n);
				for (var q = n; q <= r; ++q) {
					p.append(h.createLinkItem(q))
				}
			};
		j === d && (j = h.options.maxNavCount || 10), l(1), k > 1 && (k <= j ? l(2, k) : e < j - 2 ? (l(2, j - 2), l(!1), l(k)) : e > k - j + 2 ? (l(!1), l(k - j + 2, k)) : (l(!1), l(e - Math.ceil((j - 4) / 2), e + Math.floor((j - 4) / 2)), l(!1), l(k)))
	}, c.prototype.createGoto = function() {
		var j = this,
			h = this.state,
			k = b('<div class="input-group pager-goto" style="width: ' + (35 + 9 * (h.page + "").length + 25 + 12 * j.lang["goto"].length) + 'px"><input value="' + h.page + '" type="number" min="1" max="' + h.totalPage + '" placeholder="' + h.page + '" class="form-control pager-goto-input"><span class="input-group-btn"><button class="btn pager-goto-btn" type="button">' + j.lang["goto"] + "</button></span></div>");
		return k
	}, c.prototype.createSizeMenu = function() {
		var m = this,
			j = this.state,
			q = b('<ul class="dropdown-menu"></ul>'),
			p = m.options.pageSizeOptions;
		"string" == typeof p && (p = p.split(","));
		for (var k = 0; k < p.length; ++k) {
			var h = p[k];
			"string" == typeof h && (h = parseInt(h));
			var l = b('<li><a href="###" data-size="' + h + '">' + h + "</a></li>").toggleClass("active", h === j.recPerPage);
			q.append(l)
		}
		return b('<div class="btn-group pager-size-menu"><button type="button" class="btn dropdown-toggle" data-toggle="dropdown">' + m.lang.pageSize.format(j) + ' <span class="caret"></span></button></div>').addClass(m.options.menuDirection).append(q)
	}, c.prototype.createElement = function(l, j, p) {
		var m = this,
			k = b.proxy(m.createLinkItem, m),
			h = m.lang;
		switch (l) {
		case "prev":
			return k(p.prev, h.prev);
		case "prev_icon":
			return k(p.prev, '<i class="icon ' + m.options.prevIcon + '"></i>');
		case "next":
			return k(p.next, h.next);
		case "next_icon":
			return k(p.next, '<i class="icon ' + m.options.nextIcon + '"></i>');
		case "first":
			return k(1, h.first);
		case "first_icon":
			return k(1, '<i class="icon ' + m.options.firstIcon + '"></i>');
		case "last":
			return k(p.totalPage, h.last);
		case "last_icon":
			return k(p.totalPage, '<i class="icon ' + m.options.lastIcon + '"></i>');
		case "space":
		case "|":
			return b('<li class="space" />');
		case "nav":
		case "pages":
			return void m.createNavItems();
		case "total_text":
			return b(('<div class="pager-label">' + h.totalCount + "</div>").format(p));
		case "page_text":
			return b(('<div class="pager-label">' + h.pageOf + "</div>").format(p));
		case "total_page_text":
			return b(('<div class="pager-label">' + h.totalPage + "</div>").format(p));
		case "page_of_total_text":
			return b(('<div class="pager-label">' + h.pageOfTotal + "</div>").format(p));
		case "page_size_text":
			return b(('<div class="pager-label">' + h.pageSize + "</div>").format(p));
		case "items_range_text":
			return b(('<div class="pager-label">' + h.itemsRange + "</div>").format(p));
		case "goto":
			return m.createGoto();
		case "size_menu":
			return m.createSizeMenu();
		default:
			return b("<li/>").html(l.format(p))
		}
	}, c.prototype.createLink = function(e, j) {
		e === d && (e = this.state.page), j === d && (j = this.state);
		var h = this.options.linkCreator;
		return "string" == typeof h ? h.format(b.extend({}, j, {
			page: e
		})) : b.isFunction(h) ? h(e, j) : "#page=" + e
	}, c.prototype.render = function(v) {
		var q = this,
			m = q.state,
			k = q.options.elementCreator || q.createElement,
			y = b.isPlainObject(k);
		v = v || q.elements || q.options.elements, "string" == typeof v && (v = v.split(",")), q.elements = v, q.$.empty();
		for (var x = 0; x < v.length; ++x) {
			var j = b.trim(v[x]),
				p = y ? k[j] || k : k,
				u = p.call(q, j, q.$, m);
			u === !1 && (u = q.createElement(j, q.$, m)), u instanceof b && ("LI" !== u[0].tagName && (u = b("<li/>").append(u)), q.$.append(u))
		}
		var w = null;
		return q.$.children("li").each(function() {
			var l = b(this),
				h = !! l.children(".pager-item").length;
			w ? w.toggleClass("pager-item-right", !h) : h && l.addClass("pager-item-left"), w = h ? l : null
		}), w && w.addClass("pager-item-right"), q.$.callComEvent(q, "onRender", [m]), q
	}, c.DEFAULTS = b.extend({
	...
	}, c.NAME = a, b.fn.pager.Constructor = c, b(function() {
		b('[data-ride="pager"]').pager()
	})
}(jQuery, void 0), +
function(b) {
	var c = "zui.tab",
	...
	a.prototype.show = function() {
		var f = this.element,
			k = f.closest("ul:not(.dropdown-menu)"),
			j = f.attr("data-target") || f.attr("data-tab");
		if (j || (j = f.attr("href"), j = j && j.replace(/.*(?=#[^\s]*$)/, "")), !f.parent("li").hasClass("active")) {
			var g = k.find(".active:last a")[0],
				e = b.Event("show." + c, {
					relatedTarget: g
				});
			if (f.trigger(e), !e.isDefaultPrevented()) {
				var h = b(j);
				this.activate(f.parent("li"), k), this.activate(h, h.parent(), function() {
					f.trigger({
						type: "shown." + c,
						relatedTarget: g
					})
	...
	}, a.prototype.activate = function(j, g, l) {
		function k() {
			h.removeClass("active").find("> .dropdown-menu > .active").removeClass("active"), j.addClass("active"), f ? (j[0].offsetWidth, j.addClass("in")) : j.removeClass("fade"), j.parent(".dropdown-menu") && j.closest("li.dropdown").addClass("active"), l && l()
		}
		var h = g.find("> .active"),
			f = l && b.support.transition && h.hasClass("fade");
		f ? h.one(b.support.transition.end, k).emulateTransitionEnd(150) : k(), h.removeClass("in")
	};
	var d = b.fn.tab;
	b.fn.tab = function(e) {
		return this.each(function() {
			var g = b(this),
				f = g.data(c);
			f || g.data(c, f = new a(this)), "string" == typeof e && f[e]()
		})
	}, b.fn.tab.Constructor = a, b.fn.tab.noConflict = function() {
		return b.fn.tab = d, this
	}, b(document).on("click.zui.tab.data-api", '[data-toggle="tab"], [data-tab]', function(f) {
		f.preventDefault(), b(this).tab("show")
	})
}(window.jQuery), +
function(a) {
	function b() {
...
function(b) {
	var c = "zui.collapse",
		a = function(f, g) {
			this.$element = b(f), this.options = b.extend({}, a.DEFAULTS, g), this.transitioning = null, this.options.parent && (this.$parent = b(this.options.parent)), this.options.toggle && this.toggle()
		};
	a.DEFAULTS = {
		toggle: !0
		...
		if (!this.transitioning && !this.$element.hasClass("in")) {
			var f = b.Event("show." + c);
			if (this.$element.trigger(f), !f.isDefaultPrevented()) {
				var k = this.$parent && this.$parent.find(".in");
				if (k && k.length) {
					var j = k.data(c);
					if (j && j.transitioning) {
						return
					}
					k.collapse("hide"), j || k.data(c, null)
				}
				var g = this.dimension();
				this.$element.removeClass("collapse").addClass("collapsing")[g](0), this.transitioning = 1;
				var e = function() {
						this.$element.removeClass("collapsing").addClass("in")[g]("auto"), this.transitioning = 0, this.$element.trigger("shown." + c)
					};
				if (!b.support.transition) {
					return e.call(this)
				}
				var h = b.camelCase(["scroll", g].join("-"));
				this.$element.one(b.support.transition.end, b.proxy(e, this)).emulateTransitionEnd(350)[g](this.$element[0][h])
			}
		}
	}, a.prototype.hide = function() {
		if (!this.transitioning && this.$element.hasClass("in")) {
			var e = b.Event("hide." + c);
			if (this.$element.trigger(e), !e.isDefaultPrevented()) {
				...
				var f = function() {
						this.transitioning = 0, this.$element.trigger("hidden." + c).removeClass("collapsing").addClass("collapse")
					};
				return b.support.transition ? void this.$element[g](0).one(b.support.transition.end, b.proxy(f, this)).emulateTransitionEnd(350) : f.call(this)
			}
		}
	...
	var d = b.fn.collapse;
	b.fn.collapse = function(e) {
		return this.each(function() {
			var h = b(this),
				g = h.data(c),
				f = b.extend({}, a.DEFAULTS, h.data(), "object" == typeof e && e);
			g || h.data(c, g = new a(this, f)), "string" == typeof e && g[e]()
		})
	}, b.fn.collapse.Constructor = a, b.fn.collapse.noConflict = function() {
		return b.fn.collapse = d, this
	}, b(document).on("click." + c + ".data-api", "[data-toggle=collapse]", function(k) {
		var g, f = b(this),
			u = f.attr("data-target") || k.preventDefault() || (g = f.attr("href")) && g.replace(/.*(?=#[^\s]+$)/, ""),
			q = b(u),
			e = q.data(c),
			j = e ? "toggle" : f.data(),
			m = f.attr("data-parent"),
			p = m && b(m);
		e && e.transitioning || (p && p.find('[data-toggle=collapse][data-parent="' + m + '"]').not(f).addClass("collapsed"), f[q.hasClass("in") ? "addClass" : "removeClass"]("collapsed")), q.collapse(j)
	})
}(window.jQuery), function(k, h) {
	var g = 1200,
		d = 992,
		c = 768,
		m = h(k),
		j = function() {
			var a = m.width();
			h("html").toggleClass("screen-desktop", a >= d && a < g).toggleClass("screen-desktop-wide", a >= g).toggleClass("screen-tablet", a >= c && a < d).toggleClass("screen-phone", a < c).toggleClass("device-mobile", a < d).toggleClass("device-desktop", a >= d)
		},
		b = "",
		f = navigator.userAgent;
	f.match(/(iPad|iPhone|iPod)/i) ? b += " os-ios" : f.match(/android/i) ? b += " os-android" : f.match(/Win/i) ? b += " os-windows" : f.match(/Mac/i) ? b += " os-mac" : f.match(/Linux/i) ? b += " os-linux" : f.match(/X11/i) && (b += " os-unix"), "ontouchstart" in document.documentElement && (b += " is-touchable"), h("html").addClass(b), m.resize(j), j()
	...
	}, a.prototype.tip = function(d) {
		var e = b("#browseHappyTip");
		e.length || (e = b('<div id="browseHappyTip" class="alert alert-dismissable alert-danger-inverse alert-block" style="position: relative; z-index: 99999"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><div class="container"><div class="content text-center"></div></div></div>'), e.prependTo("body")), e.find(".content").html(d || this.browseHappyTip || c[b.zui.clientLang() || "zh_cn"])
	...
	Date.ONEDAY_TICKS = 86400000, Date.prototype.format || (Date.prototype.format = function(b) {
		...
		var c = this.getLastWeekday(),
			a = c.clone().addDays(7);
		return b >= c && b < a
	}), Date.prototype.isSameYear || (Date.prototype.isSameYear = function(a) {
		return this.getFullYear() === a.getFullYear()
	}), Date.create || (Date.create = function(a) {
		return a instanceof Date || ("number" == typeof a && a < 10000000000 && (a *= 1000), a = new Date(a)), a
	}), Date.timestamp || (Date.timestamp = function(a) {
		return "number" == typeof a ? a < 10000000000 && (a *= 1000) : a = Date.create(a).getTime(), a
	})
}(), function() {
	String.prototype.format || (String.prototype.format = function(b) {
		var c = this;
		if (arguments.length > 0) {
			var a;
			if (arguments.length <= 2 && "object" == typeof b) {
				for (var f in b) {
					void 0 !== b[f] && (a = new RegExp("(" + (arguments[1] ? arguments[1].replace("0", f) : "{" + f + "}") + ")", "g"), c = c.replace(a, b[f]))
				}
			} else {
				for (var d = 0; d < arguments.length; d++) {
					void 0 !== arguments[d] && (a = new RegExp("({[" + d + "]})", "g"), c = c.replace(a, arguments[d]))
				}
			}
		}
		return c
		...
		var a = this.toString();
		(void 0 === c || c > a.length) && (c = a.length), c -= b.length;
		var d = a.indexOf(b, c);
		return d !== -1 && d === c
		...
		f = p[b](function() {
			x.each(function() {
				var c = w(this),
					...
					h = c.height(),
					d = w.data(this, m);
				a === d.w && h === d.h || c.trigger(j, [d.w = a, d.h = h])
			}), g()
		}, v[u])
	}
	var f, x = w([]),
		v = w.resize = w.extend(w.resize, {}),
		b = "setTimeout",
		j = "resize",
		m = j + "-special-event",
		u = "delay",
		q = "throttleWindow";
	v[u] = 250, v[q] = !0, w.event.special[j] = {
		setup: function() {
			if (!v[q] && this[b]) {
				return !1
			}
			var a = w(this);
			x = x.add(a), w.data(this, m, {
				w: a.width(),
				h: a.height()
			}), 1 === x.length && g()
		},
		teardown: function() {
			...
			var a = w(this);
			x = x.not(a), a.removeData(m), x.length || clearTimeout(f)
		},
		add: function(a) {
			function d(y, z, l) {
				var h = w(this),
					o = w.data(this, m) || {};
				o.w = z !== k ? z : h.width(), o.h = l !== k ? l : h.height(), c.apply(this, arguments)
			}
			if (!v[q] && this[b]) {
				return !1
			}
			var c;
			return w.isFunction(a) ? (c = a, d) : (c = a.handler, void(a.handler = d))
		}
	}
}(jQuery, this),
/*!
...
function(a) {
	...
	var c = "zui.searchBox",
		a = function(h, k) {
			var j = this;
			j.name = name, j.$ = b(h), j.options = k = b.extend({}, a.DEFAULTS, j.$.data(), k);
			var f = j.$.is(k.inputSelector) ? j.$ : j.$.find(k.inputSelector);
			if (f.length) {
				var d = function() {
						j.changeTimer && (clearTimeout(j.changeTimer), j.changeTimer = null)
					},
					g = function() {
						d();
						var l = j.getSearch();
						if (l !== j.lastValue) {
							var m = "" === l;
							f.toggleClass("empty", m), j.$.callComEvent(j, "onSearchChange", [l, m]), j.lastValue = l
						}
					};
				j.$input = f = f.first(), f.on(k.listenEvent, function(e) {
					j.changeTimer = setTimeout(function() {
						g()
					}, k.changeDelay)
				}).on("focus", function(e) {
					f.addClass("focus"), j.$.callComEvent(j, "onFocus", [e])
				}).on("blur", function(e) {
					f.removeClass("focus"), j.$.callComEvent(j, "onBlur", [e])
				}).on("keydown", function(m) {
					var o = 0,
						l = m.which;
					27 === l && k.escToClear ? (this.setSearch("", !0), g(), o = 1) : 13 === l && k.onPressEnter && (g(), j.$.callComEvent(j, "onPressEnter", [m]));
					var n = j.$.callComEvent(j, "onKeyDown", [m]);
					n === !1 && (o = 1), o && m.preventDefault()
				}), j.$.on("click", ".search-clear-btn", function(e) {
					j.setSearch("", !0), g(), j.focus(), e.preventDefault()
				}), g()
			} else {
	...
	a.DEFAULTS = {
		...
		return this.$input && b.trim(this.$input.val())
	}, a.prototype.setSearch = function(f, g) {
		var d = this.$input;
		d && (d.val(f), g || d.trigger("change"))
	}, a.prototype.focus = function() {
		this.$input && this.$input.focus()
	}, b.fn.searchBox = function(d) {
		return this.each(function() {
			var g = b(this),
				f = g.data(c),
				e = "object" == typeof d && d;
	...
	}, a.NAME = c, b.fn.searchBox.Constructor = a
}(jQuery), function(b, d) {
	var a = "zui.draggable",
		g = {
			container: "body",
		...
		c = function(k, h) {
			var j = this;
			j.$ = b(k), j.id = f++, j.options = b.extend({}, g, j.$.data(), h), j.init()
		};
	c.DEFAULTS = g, c.NAME = a, c.prototype.init = function() {
		var H, G, B, R, E, J = this,
			L = J.$,
			P = "before",
			O = "drag",
			A = "finish",
			...
			D = function(k) {
				...
				q.removeClass("drag-ready").addClass("dragging"), z.move && (j ? z.move(m, q) : q.css(m)), z[O] && z[O]({
					event: k,
					element: q,
					startOffset: B,
					pos: m,
				...
				}), R.x = l, R.y = h, z.stopPropagation && k.stopPropagation()
			},
			S = function(h) {
				if (b(d).off(F), !E) {
				...
				q.removeClass("drag-ready dragging"), z.move && (j ? z.move(k, q) : q.css(k)), z[A] && z[A]({
					event: h,
					element: q,
					startOffset: B,
					pos: k,
				...
				}), h.preventDefault(), z.stopPropagation && h.stopPropagation()
			},
			K = function(n) {
				var k = b.zui.getMouseButtonCode(z.mouseButton);
				if (!(k > -1 && n.button !== k)) {
					var o = b(this);
					if (e && (q = Q ? o.closest(e) : o), z[P]) {
						var s = z[P]({
							event: n,
							element: q
						});
					...
					G = m.offset(), H = {
						...
						x: n.pageX - r.left + G.left,
						y: n.pageY - r.top + G.top
					}, R = b.extend({}, H), E = !1, q.addClass("drag-ready"), n.preventDefault(), z.stopPropagation && n.stopPropagation(), b(d).on(I, D).on(M, S)
				}
			};
		Q ? L.on(N, Q, K) : e ? L.on(N, e, K) : L.on(N, K)
	}, c.prototype.destroy = function() {
		var e = "." + a + "." + this.id;
		this.$.off(e), b(d).off(e), this.$.data(a, null)
	}, b.fn.draggable = function(h) {
		return this.each(function() {
			var k = b(this),
				j = k.data(a),
				e = "object" == typeof h && h;
	...
	var j = "zui.droppable",
		...
		b = function(k, a) {
			var l = this;
			l.id = f++, l.$ = d(k), l.options = d.extend({}, h, l.$.data(), a), l.init()
		};
	b.DEFAULTS = h, b.NAME = j, b.prototype.trigger = function(k, a) {
		return d.zui.callEvent(this.options[k], a, this)
	}, b.prototype.init = function() {
		var ad, Y, an, aa, af, ah, al, ak, V, ac, aj, ai, ae, R = this,
			K = R.$,
			am = R.options,
			Q = am.deviation,
			O = "." + j + "." + R.id,
			...
			H = K,
			J = !1,
			N = G ? d(am.container).first() : e ? K : d("body"),
			n = function(l) {
				if (J && (aj = {
					left: l.pageX,
					top: l.pageY
				}, !(c.abs(aj.left - ak.left) < Q && c.abs(aj.top - ak.top) < Q))) {
					if (null === an) {
						var s = N.css("position");
						"absolute" != s && "relative" != s && "fixed" != s && (ah = s, N.css("position", "relative")), an = H.clone().removeClass("drag-from").addClass("drag-shadow").css({
							position: "absolute",
							width: H.outerWidth(),
							transition: "none"
						}).appendTo(N), H.addClass("dragging"), R.trigger("start", {
							event: l,
							element: H,
							targets: ad
						})
					...
					an.css(k), d.extend(ai, aj);
					var p = !1;
					aa = !1, X || ad.removeClass(U);
					var a = null;
					if (ad.each(function() {
						var w = d(this),
							u = w.offset(),
							y = w.outerWidth(),
							x = w.outerHeight(),
							v = u.left + am.sensorOffsetX,
							m = u.top + am.sensorOffsetY;
						if (aj.left > v && aj.top > m && aj.left < v + y && aj.top < m + x && (a && a.removeClass(U), a = w, !am.nested)) {
							return !1
						}
					}), a) {
						aa = !0;
						var o = a.data("id");
						H.data("id") != o && (af = !1), (null === Y || Y.data("id") !== o && !af) && (p = !0), Y = a, X && ad.removeClass(U), Y.addClass(U)
					}
					X ? null !== Y && Y.length && (aa = !0) : (H.toggleClass("drop-in", aa), an.toggleClass("drop-in", aa)), B && B(H, Y) === !1 || R.trigger("drag", {
						event: l,
						isIn: aa,
						target: Y,
						element: H,
			...
			W = function(a) {
				if (d(g).off(O), clearTimeout(ae), J) {
					if (J = !1, ah && N.css("position", ah), null === an) {
						return H.removeClass("drag-from"), void R.trigger("always", {
							event: a,
							cancel: !0
						})
					}
					...
					var m = {
						...
						k = {
							left: aj.left - ai.left,
							top: aj.top - ai.top
						};
					ai.left = aj.left, ai.top = aj.top;
					var l = {
						event: a,
						isIn: aa,
						target: Y,
					...
					o = R.trigger("beforeDrop", l), o && aa && R.trigger("drop", l), ad.removeClass(U), H.removeClass("dragging").removeClass("drag-from"), an.remove(), an = null, R.trigger("finish", l), R.trigger("always", l), a && a.preventDefault()
				}
			},
			ab = function(a) {
				var l = d.zui.getMouseButtonCode(am.mouseButton);
				if (!(l > -1 && a.button !== l)) {
					var k = d(this);
					e && (H = q ? k.closest(e) : k), H.hasClass("drag-shadow") || am.before && am.before({
						event: a,
						element: H
					}) === !1 || (J = !0, ad = d.isFunction(am.target) ? am.target(H, K) : N.find(am.target), Y = null, an = null, aa = !1, af = !0, ah = null, al = H.offset(), V = N.offset(), V.top = V.top - N.scrollTop(), V.left = V.left - N.scrollLeft(), ak = {
						left: a.pageX,
						top: a.pageY
					}, ai = d.extend({}, ak), ac = {
						left: ak.left - al.left,
						top: ak.top - al.top
					}, H.addClass("drag-from"), d(g).on(ag, n).on(ao, W), ae = setTimeout(function() {
						d(g).on(Z, W)
					}, 10), a.preventDefault(), am.stopPropagation && a.stopPropagation())
				}
			};
		q ? K.on(Z, q, ab) : e ? K.on(Z, e, ab) : K.on(Z, ab)
	}, b.prototype.destroy = function() {
		var a = "." + j + "." + this.id;
		this.$.off(a), d(g).off(a), this.$.data(j, null)
		...
		return this.each(function() {
			var e = d(this),
				l = e.data(j),
				k = "object" == typeof a && a;
			l || e.data(j, l = new b(this, k)), "string" == typeof a && l[a]()
		})
	}, d.fn.droppable.Constructor = b
...
function(d, g) {
	function c(l, a, k) {
		return this.each(function() {
			var m = d(this),
				n = m.data(j),
				e = d.extend({}, h.DEFAULTS, m.data(), "object" == typeof l && l);
			n || m.data(j, n = new h(this, e)), "string" == typeof l ? n[l](a, k) : e.show && n.show(a, k)
		})
	}
	var j = "zui.modal",
		h = function(a, k) {
			var e = this;
			e.options = k, e.$body = d(document.body), e.$element = d(a), e.$backdrop = e.isShown = null, e.scrollbarWidth = 0, k.moveable === g && (e.options.moveable = e.$element.hasClass("modal-moveable")), k.remote && e.$element.find(".modal-content").load(k.remote, function() {
				e.$element.trigger("loaded." + j)
			})
		};
	h.VERSION = "3.2.0", h.TRANSITION_DURATION = 300, h.BACKDROP_TRANSITION_DURATION = 150, h.DEFAULTS = {
		backdrop: !0,
		keyboard: !0,
		show: !0,
		position: "fit"
	...
	var f = function(k, a) {
			var l = d(window);
			a.left = Math.max(0, Math.min(a.left, l.width() - k.outerWidth())), a.top = Math.max(0, Math.min(a.top, l.height() - k.outerHeight())), k.css(a)
		};
	h.prototype.toggle = function(a, k) {
		return this.isShown ? this.hide() : this.show(a, k)
	}, h.prototype.ajustPosition = function(q) {
		var m = this,
			y = m.options;
		if (q === g && (q = y.position), q !== g && null !== q) {
			d.isFunction(q) && (q = q(m));
			var e = m.$element.find(".modal-dialog"),
				n = d(window).height(),
				s = {
					maxHeight: "initial",
					overflow: "visible"
				},
				x = e.find(".modal-body").css(s);
			if (y.scrollInside) {
				var w = y.headerHeight;
				"number" != typeof w ? w = e.find(".modal-header").height() : d.isFunction(w) && (w = w($header)), s.maxHeight = n - w, x.outerHeight() > s.maxHeight && (s.overflow = "auto")
			}
			x.css(s);
			var z = Math.max(0, (n - e.outerHeight()) / 2);
			if ("fit" === q ? q = {
				top: z > 50 ? Math.floor(2 * z / 3) : z
			} : "center" === q ? q = {
				top: z
			} : d.isPlainObject(q) || (q = {
				top: q
			}), e.hasClass("modal-moveable")) {
				var k = null,
					v = y.rememberPos;
				v && (v === !0 ? k = m.$element.data("modal-pos") : d.zui.store && (k = d.zui.store.pageGet(j + ".rememberPos." + v))), q = d.extend(q, {
					left: Math.max(0, (d(window).width() - e.outerWidth()) / 2)
				}, k), "inside" === y.moveable ? f(e, q) : e.css(q)
			} else {
				e.css(q)
			}
		}
	}, h.prototype.setMoveale = function() {
		d.fn.draggable || console.error("Moveable modal requires draggable.js.");
		var k = this,
			a = k.options,
			l = k.$element.find(".modal-dialog").removeClass("modal-dragged");
		l.toggleClass("modal-moveable", !! a.moveable), k.$element.data("modal-moveable-setup") || l.draggable({
			container: k.$element,
			handle: ".modal-header",
			before: function() {
				var e = l.css("margin-top");
				e && "0px" !== e && l.css("top", e).css("margin-top", "").addClass("modal-dragged")
			},
			finish: function(m) {
				var e = a.rememberPos;
				e && (k.$element.data("modal-pos", m.pos), d.zui.store && e !== !0 && d.zui.store.pageSet(j + ".rememberPos." + e, m.pos))
			},
			move: "inside" !== a.moveable ||
			function(e) {
				f(l, e)
			}
		})
	}, h.prototype.show = function(n, l) {
		var m = this,
			k = d.Event("show." + j, {
				relatedTarget: n
			});
		m.$element.trigger(k), m.$element.toggleClass("modal-scroll-inside", !! m.options.scrollInside), m.isShown || k.isDefaultPrevented() || (m.isShown = !0, m.options.moveable && m.setMoveale(), m.checkScrollbar(), m.options.backdrop !== !1 && (m.$body.addClass("modal-open"), m.setScrollbar()), m.escape(), m.$element.on("click.dismiss." + j, '[data-dismiss="modal"]', function(a) {
			m.hide(), a.stopPropagation()
		}), m.backdrop(function() {
			var e = d.support.transition && m.$element.hasClass("fade");
			m.$element.parent().length || m.$element.appendTo(m.$body), m.$element.show().scrollTop(0), e && m.$element[0].offsetWidth, m.$element.addClass("in").attr("aria-hidden", !1), m.ajustPosition(l), m.enforceFocus();
			var o = d.Event("shown." + j, {
				relatedTarget: n
			});
			e ? m.$element.find(".modal-dialog").one("bsTransitionEnd", function() {
				m.$element.trigger("focus").trigger(o)
			}).emulateTransitionEnd(h.TRANSITION_DURATION) : m.$element.trigger("focus").trigger(o)
		}))
	}, h.prototype.hide = function(k) {
		k && k.preventDefault();
		var a = this;
		k = d.Event("hide." + j), a.$element.trigger(k), a.isShown && !k.isDefaultPrevented() && (a.isShown = !1, a.options.backdrop !== !1 && (a.$body.removeClass("modal-open"), a.resetScrollbar()), a.escape(), d(document).off("focusin." + j), a.$element.removeClass("in").attr("aria-hidden", !0).off("click.dismiss." + j), d.support.transition && a.$element.hasClass("fade") ? a.$element.one("bsTransitionEnd", d.proxy(a.hideModal, a)).emulateTransitionEnd(h.TRANSITION_DURATION) : a.hideModal())
	}, h.prototype.enforceFocus = function() {
		d(document).off("focusin." + j).on("focusin." + j, d.proxy(function(a) {
			this.$element[0] === a.target || this.$element.has(a.target).length || this.$element.trigger("focus")
		}, this))
	}, h.prototype.escape = function() {
		this.isShown && this.options.keyboard ? d(document).on("keydown.dismiss." + j, d.proxy(function(a) {
			if (27 == a.which) {
				var k = d.Event("escaping." + j),
					e = this.$element.triggerHandler(k, "esc");
				if (e != g && !e) {
					return
				}
				this.hide()
			}
		}, this)) : this.isShown || d(document).off("keydown.dismiss." + j)
	}, h.prototype.hideModal = function() {
		var a = this;
		this.$element.hide(), this.backdrop(function() {
			a.$element.trigger("hidden." + j)
		})
	}, h.prototype.removeBackdrop = function() {
		this.$backdrop && this.$backdrop.remove(), this.$backdrop = null
	}, h.prototype.backdrop = function(o) {
		var l = this,
			m = this.$element.hasClass("fade") ? "fade" : "";
		if (this.isShown && this.options.backdrop) {
			var k = d.support.transition && m;
			if (this.$backdrop = d('<div class="modal-backdrop ' + m + '" />').appendTo(this.$body), this.$element.on("mousedown.dismiss." + j, d.proxy(function(a) {
				a.target === a.currentTarget && ("static" == this.options.backdrop ? this.$element[0].focus.call(this.$element[0]) : this.hide.call(this))
			}, this)), k && this.$backdrop[0].offsetWidth, this.$backdrop.addClass("in"), !o) {
				return
			}
			k ? this.$backdrop.one("bsTransitionEnd", o).emulateTransitionEnd(h.BACKDROP_TRANSITION_DURATION) : o()
		} else {
			if (!this.isShown && this.$backdrop) {
				this.$backdrop.removeClass("in");
				var n = function() {
						l.removeBackdrop(), o && o()
					};
				d.support.transition && this.$element.hasClass("fade") ? this.$backdrop.one("bsTransitionEnd", n).emulateTransitionEnd(h.BACKDROP_TRANSITION_DURATION) : n()
			} else {
				o && o()
			}
		}
	...
	var b = d.fn.modal;
	d.fn.modal = c, d.fn.modal.Constructor = h, d.fn.modal.noConflict = function() {
		return d.fn.modal = b, this
	}, d(document).on("click." + j + ".data-api", '[data-toggle="modal"]', function(q) {
		var u = d(this),
			n = u.attr("href"),
			m = null;
		try {
			m = d(u.attr("data-target") || n && n.replace(/.*(?=#[^\s]+$)/, ""))
		} catch (p) {
			return
		}
		if (m.length) {
			var k = m.data(j) ? "toggle" : d.extend({
				remote: !/#/.test(n) && n
			}, m.data(), u.data());
			u.is("a") && q.preventDefault(), m.one("show." + j, function(a) {
				a.isDefaultPrevented() || m.one("hidden." + j, function() {
					u.is(":visible") && u.trigger("focus")
				})
			}), c.call(m, k, this, u.data("position"))
		}
	})
}(jQuery, void 0), function(w, p, k) {
	if (!w.fn.modal) {
		throw new Error("Modal trigger requires modal.js")
	}
	var g = "zui.modaltrigger",
		f = "ajax",
		x = ".zui.modal",
		v = "string",
		b = function(c, a) {
			c = w.extend({}, b.DEFAULTS, w.ModalTriggerDefaults, a ? a.data() : null, c), this.isShown, this.$trigger = a, this.options = c, this.id = w.zui.uuid()
		};
	b.DEFAULTS = {
		type: "custom",
		height: "auto",
		...
		var o = this;
		if (e.url && (!e.type || e.type != f && "iframe" != e.type) && (e.type = f), e.remote) {
			e.type = f, typeof e.remote === v && (e.url = e.remote)
		} else {
			if (e.iframe) {
				e.type = "iframe", typeof e.iframe === v && (e.url = e.iframe)
			} else {
				if (e.custom && (e.type = "custom", typeof e.custom === v)) {
					var a;
					try {
						a = w(e.custom)
					} catch (n) {}
					a && a.length ? e.custom = a : w.isFunction(p[e.custom]) && (e.custom = p[e.custom])
				}
			}
		}
		var y = w("#" + e.name);
		y.length && (o.isShown || y.off(x), y.remove()), y = w('<div id="' + e.name + '" class="modal modal-trigger ' + (e.className || "") + '">' + ("string" == typeof e.loadingIcon && 0 === e.loadingIcon.indexOf("icon-") ? '<div class="icon icon-spin loader ' + e.loadingIcon + '"></div>' : e.loadingIcon) + '<div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button class="close" data-dismiss="modal">×</button><h4 class="modal-title"><i class="modal-icon"></i> <span class="modal-title-name"></span></h4></div><div class="modal-body"></div></div></div></div>').appendTo("body").data(g, o);
		var s = function(c, h) {
				var d = e[c];
				w.isFunction(d) && y.on(h + x, d)
			};
		s("onShow", "show"), s("shown", "shown"), s("onHide", "hide"), s("hidden", "hidden"), s("loaded", "loaded"), y.on("shown" + x, function() {
			o.isShown = !0
		}).on("hidden" + x, function() {
			o.isShown = !1
		}), this.$modal = y, this.$dialog = y.find(".modal-dialog"), e.mergeOptions && (this.options = e)
	}, b.prototype.show = function(z) {
		var J = w.extend({}, this.options, {
		...
		var a = this,
			...
			A = this.$dialog,
			E = J.custom,
			D = A.find(".modal-body").css("padding", ""),
			I = A.find(".modal-header"),
			e = A.find(".modal-content");
		o.toggleClass("fade", J.fade).addClass(J.className).toggleClass("modal-loading", !this.isShown).toggleClass("modal-scroll-inside", !! J.scrollInside), A.toggleClass("modal-md", "md" === J.size).toggleClass("modal-sm", "sm" === J.size).toggleClass("modal-lg", "lg" === J.size).toggleClass("modal-fullscreen", "fullscreen" === J.size), I.toggle(J.showHeader), I.find(".modal-icon").attr("class", "modal-icon icon-" + J.icon), I.find(".modal-title-name").text(J.title || ""), J.size && "fullscreen" === J.size && (J.width = "", J.height = "");
		var C = function() {
				clearTimeout(this.resizeTask), this.resizeTask = setTimeout(function() {
					a.ajustPosition(J.position)
				}, 100)
			},
			B = function(c, d) {
				return "undefined" == typeof c && (c = J.delay), setTimeout(function() {
					A = o.find(".modal-dialog"), J.width && "auto" != J.width && A.css("width", J.width), J.height && "auto" != J.height && (A.css("height", J.height), "iframe" === J.type && D.css("height", A.height() - I.outerHeight())), a.ajustPosition(J.position), o.removeClass("modal-loading"), "iframe" != J.type && A.off("resize." + g).on("resize." + g, C), d && d()
				}, c)
			};
		if ("custom" === J.type && E) {
			if (w.isFunction(E)) {
				...
				typeof n === v && (D.html(n), B())
			} else {
				E instanceof w ? (D.html(w("<div>").append(E.clone()).html()), B()) : (D.html(E), B())
			}
		} else {
			if (J.url) {
				var H = function() {
						var c = o.callComEvent(a, "broken");
						c && (D.html(c), B())
					};
				if (o.attr("ref", J.url), "iframe" === J.type) {
					o.addClass("modal-iframe"), this.firstLoad = !0;
					var G = "iframe-" + J.name;
					I.detach(), D.detach(), e.empty().append(I).append(D), D.css("padding", 0).html('<iframe id="' + G + '" name="' + G + '" src="' + J.url + '" frameborder="no"  allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"  allowtransparency="true" scrolling="auto" style="width: 100%; height: 100%; left: 0px;"></iframe>'), J.waittime > 0 && (a.waitTimeout = B(J.waittime, H));
					var F = document.getElementById(G);
					F.onload = F.onreadystatechange = function() {
						var l = !! J.scrollInside;
						if (a.firstLoad && o.addClass("modal-loading"), !this.readyState || "complete" == this.readyState) {
							a.firstLoad = !1, J.waittime > 0 && clearTimeout(a.waitTimeout);
							try {
								o.attr("ref", F.contentWindow.location.href);
								var y = p.frames[G].$;
								if (y && "auto" === J.height && "fullscreen" != J.size) {
									var d = y("body").addClass("body-modal").toggleClass("body-modal-scroll-inside", l);
									J.iframeBodyClass && d.addClass(J.iframeBodyClass);
									var r = [],
										K = function(N) {
											o.removeClass("fade");
											var L = d.outerHeight();
											if (N === !0 && J.onlyIncreaseHeight && (L = Math.max(L, D.data("minModalHeight") || 0), D.data("minModalHeight", L)), l) {
												var h = J.headerHeight;
												"number" != typeof h ? h = I.height() : w.isFunction(h) && (h = h(I));
												var M = w(p).height();
												L = Math.min(L, M - h)
											}
											for (r.length > 1 && L === r[0] && (L = Math.max(L, r[1])), r.push(L); r.length > 2;) {
												r.shift()
											}
											D.css("height", L), J.fade && o.addClass("fade"), B()
										};
									o.callComEvent(a, "loaded", {
										modalType: "iframe",
										jQuery: y
									}), setTimeout(K, 100), d.off("resize." + g).on("resize." + g, K), l && w(p).off("resize." + g).on("resize." + g, K)
								} else {
									B()
								}
							} catch (s) {
					...
					w.ajax(w.extend({
						...
						success: function(c) {
							try {
								var h = w(c);
								h.filter(".modal-dialog").length ? A.replaceWith(h) : h.filter(".modal-content").length ? A.find(".modal-content").replaceWith(h) : D.wrapInner(h)
							} catch (d) {
								p.console && p.console.warn && console.warn("ZUI: Cannot recogernize remote content.", {
									error: d,
									data: c
								}), o.html(c)
							}
		...
		o.modal({
			...
			backdrop: J.backdrop,
			moveable: J.moveable,
			rememberPos: J.rememberPos,
			keyboard: J.keyboard,
			scrollInside: J.scrollInside
		})
	}, b.prototype.close = function(a, d) {
		var c = this;
		(a || d) && c.$modal.on("hidden" + x, function() {
			w.isFunction(a) && a(), typeof d === v && d.length && !c.$modal.data("cancel-reload") && ("this" === d ? p.location.reload() : p.location = d)
		}), c.$modal.modal("hide")
	}, b.prototype.toggle = function(a) {
		this.isShown ? this.close() : this.show(a)
	}, b.prototype.ajustPosition = function(a) {
		a = a === k ? this.options.position : a, w.isFunction(a) && (a = a(this)), this.$modal.modal("ajustPosition", a)
	}, w.zui({
		ModalTrigger: b,
		modalTrigger: new b
	}), w.fn.modalTrigger = function(c, a) {
		return w(this).each(function() {
			var h = w(this),
				...
				d = w.extend({
					...
					url: h.attr("href"),
					type: h.hasClass("iframe") ? "iframe" : ""
				}, h.data(), w.isPlainObject(c) && c);
			e || h.data(g, e = new b(d, h)), typeof c == v ? e[c](a) : d.show && e.show(a), h.on((d.trigger || "click") + ".toggle." + g, function(l) {
				d = w.extend(d, {
					url: h.attr("href") || h.attr("data-url") || h.data("url") || d.url
				}), e.toggle(d), h.is("a") && l.preventDefault()
			})
		})
	};
	var j = w.fn.modal;
	w.fn.modal = function(c, a) {
		return w(this).each(function() {
			var d = w(this);
			d.hasClass("modal") ? j.call(d, c, a) : d.modalTrigger(c, a)
		})
	}, w.fn.modal.bs = j;
	var m = function(a) {
			return a = w(a ? a : ".modal.modal-trigger"), a && a instanceof w ? a : null
		},
		u = function(d, l, e) {
			var c = d;
			if (w.isFunction(d)) {
			...
			d = m(d), d && d.length ? d.each(function() {
				w(this).data(g).close(l, e)
			}) : w("body").hasClass("modal-open") || w(".modal.in").length || w("body").hasClass("body-modal") && p.parent.$.zui.closeModal(c, l, e)
		},
		q = function(a, c) {
			c = m(c), c && c.length && c.modal("ajustPosition", a)
		};
	w.zui({
		closeModal: u,
		ajustModalPosition: q
	}), w(document).on("click." + g + ".data-api", '[data-toggle="modal"]', function(l) {
		var d = w(this),
			n = d.attr("href"),
			h = null;
		try {
			h = w(d.attr("data-target") || n && n.replace(/.*(?=#[^\s]+$)/, ""))
		} catch (c) {}
		h && h.length || (d.data(g) ? d.trigger(".toggle." + g) : d.modalTrigger({
			show: !0
		})), d.is("a") && l.preventDefault()
	}).on("click." + g + ".data-api", '[data-dismiss="modal"]', function() {
		w.zui.closeModal()
	})
...
function(b) {
	...
	c.DEFAULTS = {
		...
		html: !1,
		container: !1
	}, c.prototype.init = function(k, g, p) {
		this.enabled = !0, this.type = k, this.$element = b(g), this.options = this.getOptions(p);
		for (var m = this.options.trigger.split(" "), h = m.length; h--;) {
			var f = m[h];
			if ("click" == f) {
				this.$element.on("click." + this.type, this.options.selector, b.proxy(this.toggle, this))
			} else {
				if ("manual" != f) {
					var j = "hover" == f ? "mouseenter" : "focus",
						d = "hover" == f ? "mouseleave" : "blur";
		...
		var f = {},
			d = this.getDefaults();
		return this._options && b.each(this._options, function(e, g) {
			d[e] != g && (f[e] = g)
		}), f
	...
	}, c.prototype.show = function(I) {
		var E = b.Event("show.zui." + this.type);
		if ((I || this.hasContent()) && this.enabled) {
			var B = this;
			if (B.$element.trigger(E), E.isDefaultPrevented()) {
				return
			}
			var A = B.tip();
			B.setContent(I), B.options.animation && A.addClass("fade");
			var w = "function" == typeof B.options.placement ? B.options.placement.call(B, A[0], B.$element[0]) : B.options.placement,
				L = /\s?auto?\s?/i,
				x = L.test(w);
			x && (w = w.replace(L, "") || "top"), A.detach().css({
				top: 0,
				left: 0,
				display: "block"
			}).addClass(w), B.options.container ? A.appendTo(B.options.container) : A.insertAfter(B.$element);
			var D = B.getPosition(),
				F = A[0].offsetWidth,
				K = A[0].offsetHeight;
			if (x) {
				var J = B.$element.parent(),
					q = w,
					z = document.documentElement.scrollTop || document.body.scrollTop,
					H = "body" == B.options.container ? window.innerWidth : J.outerWidth(),
					G = "body" == B.options.container ? window.innerHeight : J.outerHeight(),
					C = "body" == B.options.container ? 0 : J.offset().left;
				w = "bottom" == w && D.top + D.height + K - z > G ? "top" : "top" == w && D.top - z - K < 0 ? "bottom" : "right" == w && D.right + F > H ? "left" : "left" == w && D.left - F < C ? "right" : w, A.removeClass(q).addClass(w)
			}
			var k = B.getCalculatedOffset(w, D, F, K);
			B.applyPlacement(k, w);
			var j = function() {
					var d = B.hoverState;
					B.$element.trigger("shown.zui." + B.type), B.hoverState = null, "out" == d && B.leave(B)
				};
			b.support.transition && B.$tip.hasClass("fade") ? A.one("bsTransitionEnd", j).emulateTransitionEnd(150) : j()
		}
	}, c.prototype.applyPlacement = function(v, p) {
		var k, g = this.tip(),
			f = g[0].offsetWidth,
			w = g[0].offsetHeight,
			u = parseInt(g.css("margin-top"), 10),
			d = parseInt(g.css("margin-left"), 10);
		isNaN(u) && (u = 0), isNaN(d) && (d = 0), v.top = v.top + u, v.left = v.left + d, g.offset(v).addClass("in");
		var j = g[0].offsetWidth,
			m = g[0].offsetHeight;
		if ("top" == p && m != w && (k = !0, v.top = v.top + w - m), /bottom|top/.test(p)) {
			var q = 0;
			v.left < 0 && (q = v.left * -2, v.left = 0, g.offset(v), j = g[0].offsetWidth, m = g[0].offsetHeight), this.replaceArrow(q - f + j, j, "left")
		} else {
			this.replaceArrow(m - w, m, "top")
		}
		k && g.offset(v)
		...
		var d = this,
			h = this.tip(),
			g = b.Event("hide.zui." + this.type);
		if (this.$element.trigger(g), !g.isDefaultPrevented()) {
			return h.removeClass("in"), b.support.transition && this.$tip.hasClass("fade") ? h.one(b.support.transition.end, f).emulateTransitionEnd(150) : f(), this.$element.trigger("hidden.zui." + this.type), this
		}
	...
	}, c.prototype.getCalculatedOffset = function(f, g, d, h) {
		return "bottom" == f ? {
			top: g.top + g.height,
	...
	var a = b.fn.tooltip;
	b.fn.tooltip = function(d, e) {
		return this.each(function() {
			var h = b(this),
				g = h.data("zui.tooltip"),
				f = "object" == typeof d && d;
			g || h.data("zui.tooltip", g = new c(this, f)), "string" == typeof d && g[d](e)
		})
...
function(b) {
	...
	c.DEFAULTS = b.extend({}, b.fn.tooltip.Constructor.DEFAULTS, {
		...
		var f = this.tip(),
			g = this.getTarget();
		if (g) {
			return g.find(".arrow").length < 1 && f.addClass("no-arrow"), void f.html(g.html())
		}
		var d = this.getTitle(),
			h = this.getContent();
		f.find(".popover-title")[this.options.html ? "html" : "text"](d), f.find(".popover-content")[this.options.html ? "html" : "text"](h), f.removeClass("fade top bottom left right in"), this.options.tipId && f.attr("id", this.options.tipId), this.options.tipClass && f.addClass(this.options.tipClass), f.find(".popover-title").html() || f.find(".popover-title").hide()
	}, c.prototype.hasContent = function() {
		...
		var f = this.$element,
			d = this.options,
			g = f.attr("data-target") || ("function" == typeof d.target ? d.target.call(f[0]) : d.target);
		return !!g && ("$next" == g ? f.next(".popover") : b(g))
	}, c.prototype.arrow = function() {
	...
	b.fn.popover = function(d) {
		return this.each(function() {
			var g = b(this),
				f = g.data("zui.popover"),
				e = "object" == typeof d && d;
			f || g.data("zui.popover", f = new c(this, e)), "string" == typeof d && f[d]()
		})
...
function(k) {
	function h() {
		k(c).remove(), k(m).each(function(a) {
			var l = g(k(this));
			l.hasClass("open") && (l.trigger(a = k.Event("hide." + d)), a.isDefaultPrevented() || l.removeClass("open").trigger("hidden." + d))
		})
	}
	function g(l) {
		var a = l.attr("data-target");
		a || (a = l.attr("href"), a = a && /#/.test(a) && a.replace(/.*(?=#[^\s]*$)/, ""));
		var q;
		try {
			q = a && k(a)
		} catch (p) {}
		return q && q.length ? q : l.parent()
	}
	var d = "zui.dropdown",
		c = ".dropdown-backdrop",
		m = "[data-toggle=dropdown]",
		j = function(a) {
			k(a).on("click." + d, this.toggle)
		};
	j.prototype.toggle = function(p) {
		var l = k(this);
		if (!l.is(".disabled, :disabled")) {
			var e = g(l),
				n = e.hasClass("open");
			if (h(), !n) {
				if ("ontouchstart" in document.documentElement && !e.closest(".navbar-nav").length && k('<div class="dropdown-backdrop"/>').insertAfter(k(this)).on("click", h), e.trigger(p = k.Event("show." + d)), p.isDefaultPrevented()) {
					return
				}
				e.toggleClass("open").trigger("shown." + d), l.focus()
			}
			return !1
		}
	}, j.prototype.keydown = function(u) {
		if (/(38|40|27)/.test(u.keyCode)) {
			var w = k(this);
			if (u.preventDefault(), u.stopPropagation(), !w.is(".disabled, :disabled")) {
				var v = g(w),
					q = v.hasClass("open");
				if (!q || q && 27 == u.keyCode) {
					return 27 == u.which && v.find(m).focus(), w.click()
				}
				var s = k("[role=menu] li:not(.divider):visible a", v);
				if (s.length) {
					var p = s.index(s.filter(":focus"));
					38 == u.keyCode && p > 0 && p--, 40 == u.keyCode && p < s.length - 1 && p++, ~p || (p = 0), s.eq(p).focus()
				}
			}
		}
	};
	...
	k.fn.dropdown = function(a) {
		return this.each(function() {
			var e = k(this),
				l = e.data("dropdown");
			l || e.data("dropdown", l = new j(this)), "string" == typeof a && l[a].call(e)
		})
	}, k.fn.dropdown.Constructor = j, k.fn.dropdown.noConflict = function() {
		return k.fn.dropdown = b, this
	};
	var f = d + ".data-api";
	k(document).on("click." + f, h).on("click." + f, ".dropdown form", function(a) {
		a.stopPropagation()
	}).on("click." + f, m, j.prototype.toggle).on("keydown." + f, m + ", [role=menu]", j.prototype.keydown)
}(window.jQuery), function(w, p, k) {
	var g = 0,
		...
		v = {},
		b = function(z, s) {
			w.isPlainObject(z) && (s = z, z = s.message);
			var a = this;
			s = a.options = w.extend({}, x, s), a.id = s.id || g++;
			var o = v[a.id];
			o && o.destroy(), v[a.id] = a, a.message = (s.icon ? '<i class="icon-' + s.icon + ' icon"></i> ' : "") + z, a.$ = w(f.format(s)).toggleClass("fade", s.fade).toggleClass("scale", s.scale).attr("id", "messager-" + a.id), s.cssClass && a.$.addClass(s.cssClass);
			var B = !1,
				A = a.$.find(".messager-actions"),
				n = function(c) {
					var d = w('<button type="button" class="action action-' + c.name + '"/>');
					"close" === c.name && d.addClass("close"), c.html !== k && d.html(c.html), c.icon !== k && d.append('<i class="action-icon icon-' + c.icon + '"/>'), c.text !== k && d.append('<span class="action-text">' + c.text + "</span>"), c.tooltip !== k && d.attr("title", c.tooltip).tooltip(), d.data("action", c), A.append(d)
				};
			s.actions && w.each(s.actions, function(c, d) {
				d.name === k && (d.name = c), "close" == d.name && (B = !0), n(d)
			}), !B && s.close && n({
				name: "close",
				html: "&times;"
			}), a.$.on("click", ".action", function(d) {
				var c, h = w(this).data("action");
				s.onAction && (c = s.onAction.call(this, h.name, h, a), c === !1) || w.isFunction(h.action) && (c = h.action.call(this, a), c === !1) || (a.hide(), d.stopPropagation())
			}), a.$.on("click", function(c) {
				if (s.onAction) {
					var d = s.onAction.call(this, "content", null, a);
					d === !0 && a.hide()
				}
			});
			var y = a.$.find(".messager-content").html(a.message);
			s.contentClass && y.addClass(s.contentClass), a.$.data("zui.messager", a), s.show && a.message !== k && a.show()
		};
	b.prototype.update = function(c, a) {
		var h = this,
			d = h.options;
		h.$.removeClass("messager-" + d.type), a && (d = w.extend(d, a)), h.$.addClass("messager-" + d.type), c && (h.message = (d.icon ? '<i class="icon-' + d.icon + ' icon"></i> ' : "") + c, h.$.find(".messager-content").html(h.message))
	}, b.prototype.show = function(z, y) {
		var F = this,
			E = this.options;
		if (w.isFunction(z)) {
			var e = y;
		...
		if (F.isShow) {
			return void F.hide(function() {
				F.show(z, y)
			})
		}
		F.hiding && (clearTimeout(F.hiding), F.hiding = null), F.update(z);
		var A = E.placement,
			B = w(E.parent),
			D = B.children(".messagers-holder." + A);
		if (D.length || (D = w("<div/>").attr("class", "messagers-holder " + A).appendTo(B)), D.append(F.$), "center" === A) {
			var C = w(p).height() - D.height();
			D.css("top", Math.max(-C, C / 2))
		}
		return F.$.show().addClass("in"), E.time && (F.hiding = setTimeout(function() {
			F.hide()
		}, E.time)), F.isShow = !0, y && y(), F
	}, b.prototype.hide = function(c, d) {
		c === !0 && (d = !0, c = null);
		var a = this;
		if (a.$.hasClass("in")) {
			a.$.removeClass("in");
			var h = function() {
					var l = a.$.parent();
					a.$.detach(), l.children().length || l.remove(), c && c(!0)
				};
			d ? h() : setTimeout(h, 200)
		} else {
			c && c(!1)
		}
		a.isShow = !1
	...
	var j = function() {
		...
		m = function(a, d) {
			"string" == typeof d && (d = {
				type: d
			}), d = w.extend({}, d), d.id === k && j();
			var c = v[d.id] || new b(a, d);
			return c.show(), c
		},
		u = function(a) {
...
}(jQuery, window, void 0), function(B, N, J, G) {
	function F(b) {
		if (b = b.toLowerCase(), b && P.test(b)) {
			var c;
			if (4 === b.length) {
				var a = "#";
				for (c = 1; c < 4; c += 1) {
					a += b.slice(c, c + 1).concat(b.slice(c, c + 1))
				}
				b = a
			}
			var d = [];
			for (c = 1; c < 7; c += 2) {
				d.push(Q("0x" + b.slice(c, c + 2)))
			}
			return {
				r: d[0],
				g: d[1],
		...
		throw new Error("Wrong hex string! (hex: " + b + ")")
	}
	function C(a) {
		return typeof a === M && ("transparent" === a.toLowerCase() || H[a.toLowerCase()] || P.test(B.trim(a.toLowerCase())))
	}
	function R(p) {
		function h(a) {
			return a = a < 0 ? a + 1 : a > 1 ? a - 1 : a, 6 * a < 1 ? b + (m - b) * a * 6 : 2 * a < 1 ? m : 3 * a < 2 ? b + (m - b) * (2 / 3 - a) * 6 : b
		}
		var g = p.h,
			f = p.s,
			d = p.l,
			u = p.a;
		g = K(g) % A / A, f = I(K(f)), d = I(K(d)), u = I(K(u));
		var m = d <= 0.5 ? d * (f + 1) : d + f - d * f,
			b = 2 * d - m,
			l = {
				r: h(g + 1 / 3) * O,
				g: h(g) * O,
	...
	function D(b, a, c) {
		return z(c) && (c = 0), z(a) && (a = O), N.min(N.max(b, c), a)
	}
	function I(a, b) {
		return D(a, b)
	}
	...
	var P = /^#([0-9a-fA-f]{3}|[0-9a-fA-f]{6})$/,
		...
		k = function(b, h, a, o) {
			var f = this;
			if (f.r = f.g = f.b = 0, f.a = 1, j(o) && (f.a = I(K(o), 1)), j(b) && j(h) && j(a)) {
				f.r = q(b), f.g = q(h), f.b = q(a)
			} else {
				if (j(b)) {
					var g = typeof b;
					if (g == M) {
						if (b = b.toLowerCase(), "transparent" === b) {
							f.a = 0
						} else {
							if (H[b]) {
								this.rgb(F(H[b]))
								...
								if (0 === b.indexOf("rgb")) {
									var m = b.substring(b.indexOf("(") + 1, b.lastIndexOf(")")).split(",", 4);
									f.rgb({
										r: m[0],
										g: m[1],
										b: m[2],
						...
						if ("number" == g && z(h)) {
							...
							if (g == L && j(b.r)) {
								f.r = q(b.r), j(b.g) && (f.g = q(b.g)), j(b.b) && (f.b = q(b.b)), j(b.a) && (f.a = I(K(b.a), 1))
							} else {
								if (g == L && j(b.h)) {
									var l = {
										h: I(K(b.h), A),
									...
									j(b.s) && (l.s = I(K(b.s), 1)), j(b.l) && (l.l = I(K(b.l), 1)), j(b.a) && (l.a = I(K(b.a), 1)), f.rgb(R(l))
								}
							}
						}
					}
	...
	k.prototype.rgb = function(b) {
		var c = this;
		if (j(b)) {
			if (typeof b == L) {
				j(b.r) && (c.r = q(b.r)), j(b.g) && (c.g = q(b.g)), j(b.b) && (c.b = q(b.b)), j(b.a) && (c.a = I(K(b.a), 1))
			} else {
				var a = Q(K(b));
				c.r = a, c.g = a, c.b = a
			}
	...
	}, k.prototype.hue = function(b) {
		var c = this,
			a = c.toHsl();
		return z(b) ? a.h : (a.h = I(K(b), A), c.rgb(R(a)), c)
	}, k.prototype.darken = function(b) {
		var c = this,
			a = c.toHsl();
		return a.l -= b / E, a.l = I(a.l, 1), c.rgb(R(a)), c
	}, k.prototype.clone = function() {
		var a = this;
		return new k(a.r, a.g, a.b, a.a)
	}, k.prototype.lighten = function(a) {
		return this.darken(-a)
	}, k.prototype.fade = function(a) {
		return this.a = I(a / E, 1), this
	}, k.prototype.spin = function(b) {
		var c = this.toHsl(),
			a = (c.h + b) % A;
		return c.h = a < 0 ? A + a : a, this.rgb(R(c))
		...
		if (g === u) {
			...
			switch (m = v > 0.5 ? d / (2 - g - u) : d / (g + u), g) {
			case e:
				x = (y - w) / d + (y < w ? 6 : 0);
				break;
			case y:
	...
	}, k.prototype.luma = function() {
		var b = this.r / O,
			a = this.g / O,
			c = this.b / O;
		return b = b <= 0.03928 ? b / 12.92 : N.pow((b + 0.055) / 1.055, 2.4), a = a <= 0.03928 ? a / 12.92 : N.pow((a + 0.055) / 1.055, 2.4), c = c <= 0.03928 ? c / 12.92 : N.pow((c + 0.055) / 1.055, 2.4), 0.2126 * b + 0.7152 * a + 0.0722 * c
	}, k.prototype.saturate = function(a) {
		var b = this.toHsl();
		return b.s += a / E, b.s = I(b.s), this.rgb(R(b))
	}, k.prototype.desaturate = function(a) {
		return this.saturate(-a)
	}, k.prototype.contrast = function(b, c, a) {
		if (c = z(c) ? new k(O, O, O, 1) : new k(c), b = z(b) ? new k(0, 0, 0, 1) : new k(b), b.luma() > c.luma()) {
			var d = c;
			c = b, b = d
		}
		return this.a < 0.5 ? b : (a = z(a) ? 0.43 : K(a), this.luma() < a ? c : b)
	}, k.prototype.hexStr = function() {
	...
	}, k.prototype.toCssStr = function() {
		var a = this;
		return a.a > 0 ? a.a < 1 ? "rgba(" + a.r + "," + a.g + "," + a.b + "," + a.a + ")" : a.hexStr() : "transparent"
	}, k.isColor = C, k.names = H, k.get = function(a) {
		return new k(a)
	}, B.zui({
		Color: k
	})
...
function(a) {
	"function" == typeof define && define.amd ? define(["jquery"], a) : "object" == typeof module && module.exports ? module.exports = function(c, b) {
		return "undefined" == typeof b && (b = "undefined" != typeof window ? require("jquery") : require("jquery")(c)), a(b), b
	} : a(jQuery)
}(function(t) {
	function e(e) {
		var i = e.data;
	...
	function i(e) {
		...
		if (s.clk = i, "image" === i.type) {
			if ("undefined" != typeof e.offsetX) {
				s.clk_x = e.offsetX, s.clk_y = e.offsetY
			} else {
				if ("function" == typeof t.fn.offset) {
					var a = n.offset();
					s.clk_x = e.pageX - a.left, s.clk_y = e.pageY - a.top
				} else {
					s.clk_x = e.pageX - i.offsetLeft, s.clk_y = e.pageY - i.offsetTop
				}
			}
		}
		setTimeout(function() {
	...
	t.fn.attr2 = function() {
		...
		function l(i) {
			var n, o, s = t.param(i, e.traditional).split("&"),
				...
				r = [];
			for (n = 0; n < a; n++) {
				s[n] = s[n].replace(/\+/g, " "), o = s[n].split("="), r.push([decodeURIComponent(o[0]), decodeURIComponent(o[1])])
			}
			return r
		}
		function h(i) {
		...
		function c(i) {
			...
			function s() {
				...
				try {
					if (c.extraData) {
						for (var u in c.extraData) {
							c.extraData.hasOwnProperty(u) && (t.isPlainObject(c.extraData[u]) && c.extraData[u].hasOwnProperty("name") && c.extraData[u].hasOwnProperty("value") ? h.push(t('<input type="hidden" name="' + c.extraData[u].name + '">', T).val(c.extraData[u].value).appendTo(_)[0]) : h.push(t('<input type="hidden" name="' + u + '">', T).val(c.extraData[u]).appendTo(_)[0]))
						}
					}
			...
			function r(e) {
				if (!v.aborted && !I) {
					if (z = o(m), z || (n("cannot access response document"), e = M), e === D && v) {
						return v.abort("timeout"), void k.reject(v, "timeout")
					}
					if (e === M && v) {
						return v.abort("server abort"), void k.reject(v, "error", "server abort")
					}
					if (z && z.location.href !== c.iframeSrc || x) {
						...
						try {
							...
							if (n("isXml=" + a), !a && window.opera && (null === z.body || !z.body.innerHTML) && --$) {
								return n("requeing onLoad callback, DOM not available"), void setTimeout(r, 250)
							}
							...
							if (d || c.textarea) {
								...
								if (p) {
									v.responseText = p.value, v.status = Number(p.getAttribute("status")) || v.status, v.statusText = p.getAttribute("statusText") || v.statusText
								} else {
									if (d) {
								...
								"xml" === h && !v.responseXML && v.responseText && (v.responseXML = E(v.responseText))
							}
							try {
								L = j(v, h, c)
							} catch (b) {
			...
			if (k.abort = function(t) {
				...
				for (h = 0; h < y.length; h++) {
					l = t(y[h]), a ? l.prop("disabled", !1) : l.removeAttr("disabled")
				}
			}
			...
			}, u = c.global, u && 0 === t.active++ && t.event.trigger("ajaxStart"), u && t.event.trigger("ajaxSend", [v, c]), c.beforeSend && c.beforeSend.call(c.context, v, c) === !1) {
				return c.global && t.active--, k.reject(), k
			}
			if (v.aborted) {
				return k.reject(), k
			}
		...
		if (!this.length) {
			return n("ajaxSubmit: skipping submit process - no element selected"), this
		}
		var d, u, p, f = this;
		"function" == typeof e ? e = {
			success: e
		...
		var g = {};
		if (this.trigger("form-pre-serialize", [this, e, g]), g.veto) {
		...
		if (e.beforeSubmit && e.beforeSubmit(b, this, e) === !1) {
			return n("ajaxSubmit: submit aborted via beforeSubmit callback"), this
		}
		if (this.trigger("form-submit-validate", [b, this, e, g]), g.veto) {
		...
		if (e.resetForm && C.push(function() {
			...
			e.success && (t.isArray(e.success) ? t.merge(C, e.success) : C.push(e.success))
		}
		if (e.success = function(t, i, n) {
			for (var o = e.context || this, s = 0, a = C.length; s < a; s++) {
				C[s].apply(o, [t, i, n || f, f])
			}
		}, e.error) {
			var k = e.error;
		...
		if (h && (h = t.makeArray(h)), l && (e || /(Edge|Trident)\//.test(navigator.userAgent)) && (a = t(':input[form="' + l + '"]').get(), a.length && (h = (h || []).concat(a))), !h || !h.length) {
			return o
		}
		t.isFunction(n) && (h = t.map(h, n));
		var c, d, u, p, f, g, m;
		for (c = 0, g = h.length; c < g; c++) {
			if (f = h[c], u = f.name, u && !f.disabled) {
				if (e && r.clk && "image" === f.type) {
					r.clk === f && (o.push({
						name: u,
						value: t(f).val(),
						type: f.type
					...
					if (p = t.fieldValue(f, !0), p && p.constructor === Array) {
						for (i && i.push(f), d = 0, m = p.length; d < m; d++) {
							o.push({
								name: u,
								value: p[d]
							})
						...
						if (s.fileapi && "file" === f.type) {
							...
							null !== p && "undefined" != typeof p && (i && i.push(f), o.push({
								name: u,
								value: p,
								type: f.type,
								required: f.required
		...
		return this.each(function() {
			...
			if (n) {
				...
				if (o && o.constructor === Array) {
					...
					null !== o && "undefined" != typeof o && i.push({
						name: this.name,
						value: o
					})
				}
		...
		if ("undefined" == typeof i && (i = !0), i && (!n || e.disabled || "reset" === s || "button" === s || ("checkbox" === s || "radio" === s) && !e.checked || ("submit" === s || "image" === s) && e.form && e.form.clk !== e || "select" === a && e.selectedIndex === -1)) {
			return null
		}
		if ("select" === a) {
			...
			for (var l = [], h = e.options, c = "select-one" === s, d = c ? r + 1 : h.length, u = c ? r : 0; u < d; u++) {
				...
				if (p.selected && !p.disabled) {
					var f = p.value;
					if (f || (f = p.attributes && p.attributes.value && !p.attributes.value.specified ? p.text : p.value), c) {
						return f
					}
					l.push(f)
				}
